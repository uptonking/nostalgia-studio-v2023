...
import { type LoguxActionError } from '../index';
...
import { TestClient, TestServer } from '../index';
...
it('tracks action processing', async () => {
  ...
  let unknownError = await catchError(() =>
    client.process({ type: 'UNKNOWN' }),
  );
  ...
  expect(customError2.message).toEqual(
    'Undo was with error reason, not denied',
  );
  ...
  expect(reasonError.message).toEqual(
    'Undo was with error reason, not another',
  );
...
it('tracks subscriptions', async () => {
  ...
  expect(actions3).toEqual([
    { type: 'FOO', since: { id: '1 1:0:0', time: 1 } },
  ]);
