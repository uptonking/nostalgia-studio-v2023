...
export class HLTime {
  ...
  public static parse(timestamp: string): HLTime {
    ...
    if (parts.length !== 3) {
      throw new HLTime.ParseError(
        timestamp,
        'not in format <date>_<counter>_<node>',
      );
    }
    ...
    if (isNaN(millis)) {
      throw new HLTime.ParseError(
        timestamp,
        'unable to parse date/time string to date',
      );
    } else if (isNaN(counter)) {
      throw new HLTime.ParseError(
        timestamp,
        'failed to parse counter to number',
      );
    } else if (node.length !== HLTime.NODE_PART_STR_LENGTH) {
      throw new HLTime.ParseError(
        timestamp,
        `node ID should have ${HLTime.NODE_PART_STR_LENGTH} characters`,
      );
    }
  ...
  toString(): string {
    ...
    return [
      ...
      (
        ...
        this.counter().toString(16).toUpperCase()
      ).slice(-HLTime.COUNTER_PART_STR_LENGTH),
      ('0'.repeat(HLTime.NODE_PART_STR_LENGTH) + this.node()).slice(
        -HLTime.NODE_PART_STR_LENGTH,
      ),
