...
class IDBOpenDBRequestProxy {
  ...
  upgradeOpLoggyDb(
    ...
    oplogStore.transaction.onerror = () => {
      throw new OpLoggyProxyError(
        `Failed to create OpLoggy's oplog object store in IndexedDB.`,
      );
    };
...
class OpLoggyProxyError extends Error {
  ...
  constructor(message: string) {
    ...
    // (but notes it might not work in IE <= 10): https://git.io/vHLlu
