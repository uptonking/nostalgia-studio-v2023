...
export class IDBObjectStoreProxy {
    ...
    existingObjReq.onsuccess = () => {
      ...
      try {
        const mergedPutReq = keyPath
          ? this.target.put(resolvedValue)
          : this.target.put(resolvedValue, key);
  ...
  recordOperation = (newValue: any, key?: IDBValidKey) => {
    if (key instanceof ArrayBuffer || key instanceof DataView) {
      throw new TypeError(
        `Keys of type ArrayBuffer or DataView aren't currently supported.`,
      );
    } else if (Array.isArray(key)) {
      const foundArrBuffItem = key.find(
        () => key instanceof ArrayBuffer || key instanceof DataView,
      );
      if (foundArrBuffItem) {
        throw new TypeError(
          `Keys of type ArrayBuffer or DataView aren't currently supported.`,
...
export function resolveKey(
  ...
  if (
    !resolvedKey ||
    (Array.isArray(resolvedKey) && resolvedKey.length === 0)
  ) {
...
export class PutWithoutKeyError extends Error {
  constructor(store: IDBObjectStore) {
    const formattedKeyNames = Array.isArray(store.keyPath)
      ? store.keyPath.join('", "')
      : `"${store.keyPath}"`;
    super(
