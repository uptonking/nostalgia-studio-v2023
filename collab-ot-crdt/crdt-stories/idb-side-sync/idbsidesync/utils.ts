...
export function makeClientId(): string {
  return uuid().replace(/-/g, '').slice(-HLTime.NODE_PART_STR_LENGTH);
}
...
export function throwIfInvalidOpLogEntry(thing: unknown): void {
  ...
  if (typeof candidate.store !== 'string' || candidate.store.trim() === '') {
    throw new Error(
      'Object must have "store" property set to a non-empty string',
    );
  }
  ...
  try {
    ...
    throw new Error(
      'Object must have a valid "hlcTime" property: ' + error.message,
    );
  }
...
export function isEventWithTargetError(
  thing: unknown,
): thing is EventWithTargetError {
  if (!thing) {
    return false;
  }
