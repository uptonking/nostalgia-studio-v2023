...
/*
 ...
 Spec is here: https://github.com/ottypes/json0
 Note: This is being made obsolete. It will soon be replaced by the JSON2 type.
*/

/** Checks if the passed object is an Array instance.
 * - Can't use Array.isArray because its not supported on IE8.
 */
 ...
 * the `registerSubtype` function. This allows another type to handle certain
 * operations instead of the builtin JSON type.
 */
const subtypes = {};
json.registerSubtype = (subtype) => {
  subtypes[subtype.name] = subtype;
};

json.create = (data) => (data === undefined ? null : clone(data));
...
const pathMatches = (json.pathMatches = (p1, p2, ignoreLast) => {
...
json.append = (dest, c) => {
  ...
  if (pathMatches(c.p, last.p, undefined)) {
    // handle subtype ops
...
json.transformComponent = (dest, c, otherC, type) => {
  ...
  const common = json.commonLengthForOps(otherC, c);
  const common2 = json.commonLengthForOps(c, otherC);
  let cplength = c.p.length;
  let otherCplength = otherC.p.length;
