...
const strInject = (s1, pos, s2) => {
  return s1.slice(0, pos) + s2 + s1.slice(pos);
};

/** Check that an operation component is valid. Throws if its invalid. */
const checkValidComponent = (c) => {
 ...
 */
const append = (text._append = (newOp, c) => {
  if (c.i === '' || c.d === '') return;
...
const transformPosition = (pos, c, insertAfter) => {
 ...
 */
const transformComponent = (text._tc = (dest, c, otherC, side) => {
  ...
  if (c.i != null) {
    ...
    if (otherC.i != null) {
      ...
      if (c.p >= otherC.p + otherC.d.length)
        ...
        if (newC.d !== '') {
          newC.p = transformPosition(newC.p, otherC, undefined);
          append(dest, newC);
        }
      }
    }
