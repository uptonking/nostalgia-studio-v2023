...
const log = (...args: any) => {};
function assert(pred: boolean, msg?: string): asserts pred {
...
export class ReadCursor extends Cursor {
  ...
  getKey() {
    assert(
      this.container != null,
      'Invalid call to getKey before cursor descended',
    );
  ...
  getComponent() {
    ...
    if (
      this.container &&
      this.container.length > this.idx + 1 &&
      isObject((c = this.container[this.idx + 1]))
    ) {
  ...
  *[Symbol.iterator]() {
  ...
  traverse<W extends null | WriteCursor>(
    w: W,
    fn: (c: JSONOpComponent, w: W) => void,
  ) {
  ...
  eachPick<W extends null | WriteCursor>(
    w: W,
    fn: (slot: number, w: W) => void,
  ) {
    this.traverse(w, (c, w) => {
      if (c.p != null) fn(c.p, w);
    });
  }
  eachDrop<W extends null | WriteCursor>(
    w: W,
    fn: (slot: number, w: W) => void,
  ) {
    this.traverse(w, (c, w) => {
      if (c.d != null) fn(c.d, w);
    });
  }
}
...
export class WriteCursor extends Cursor {
      ...
      if (i === this.container.length) {
        ...
        if (this.lcIdx !== -1) {
          assert(
            isGreaterKey(
              k,
              (this.container[this.lcIdx] as JSONOpList)[0] as Key,
            ),
          );
          i = this.lcIdx + 1;
          this.lcIdx = -1;
        ...
        while (
          i < this.container.length &&
          isGreaterKey(k, (this.container[i] as JSONOpList)[0] as Key)
        )
          i++;
        ...
        if (
          i < this.container.length &&
          (this.container[i] as JSONOpList)[0] === k
        ) {
  ...
  write<K extends keyof JSONOpComponent>(key: K, value: JSONOpComponent[K]) {
    ...
    assert(
      component[key] == null || component[key] === value,
  ...
  // relaxed.
  writeAtPath<K extends keyof JSONOpComponent>(
    path: Path,
    key: K,
