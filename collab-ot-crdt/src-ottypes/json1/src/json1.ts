...
const getComponent = (r?: ReadCursor | WriteCursor | null) =>
  r ? r.getComponent() : null;
...
function insertChildMut(container: Doc[] | DocObj, key: Key, value: Doc) {
  if (typeof key === 'number') {
    ...
    assert(
      (container as DocObj)[key] === undefined,
...
function registerSubtype(
  subtype: Subtype | { type: Subtype; [k: string]: any },
) {
  const _subtype = subtype.type ? subtype.type : subtype;
...
const getEdit = (c: JSONOpComponent) =>
  c.es ? c.es : c.ena != null ? c.ena : c.e;

// Type is an object or a string name.
const writeEdit = (
  w: WriteCursor,
  typeOrName: Subtype | string,
  edit: any,
  preserveNoop: boolean = false,
) => {
  ...
  if (name === 'number') {
    ...
    // TODO: Also if t1 === subtypes.number then write ena...
...
function checkValidOp(op: JSONOp) {
  ...
  const checkDescent = (descent: JSONOpList, isRoot: boolean, removed: Doc) => {
    ...
    assert(
      numDescents !== 1,
      'Operation makes multiple descents. Remove some []',
    );
  ...
  assert(
    pickedSlots.size === droppedSlots.size,
...
function apply(snapshot: Doc | undefined, op: JSONOp) {
  ...
  function drop(root: Doc, descent: JSONOpList) {
    ...
    let subDoc = root;
    let i = 0; // For reading
    const rootContainer: Doc = { root }; // This is an avoidable allocation.
    let m = 0;
    let container: Doc = rootContainer;
    let key: Key = 'root'; // For writing
    ...
    function mut() {
      for (; m < i; m++) {
        ...
        container = (container as any)[key] = shallowClone(
...
const incPrefix = () => {
  if (!RELEASE_MODE) (log as any).prefix++;
};
const decPrefix = () => {
  if (!RELEASE_MODE) (log as any).prefix--;
};
...
function transformPosition(path: Path, op: JSONOp): Path | null {
  ...
  const handleDrop = () => {
    ...
    for (; i < path.length; i++) {
      ...
      const dropAdv = advancer(
        r,
        (k, c) => (hasDrop(c) ? ~(k - dropOffset) : k - dropOffset),
        (k, c) => {
...
function compose(op1: JSONOp, op2: JSONOp) {
  ...
  function xfBoundary(
    r1Pick: ReadCursor | null,
    r1Drop: ReadCursor | null,
    r2Pick: ReadCursor | null,
    r2Drop: ReadCursor | null,
    litIn: Doc | undefined,
    rmParent: boolean,
    wd: WriteCursor,
    wp: WriteCursor,
    ...
    if (!RELEASE_MODE)
      log(
        'xfBoundary',
        litIn,
        Boolean(r1Pick) && r1Pick.getPath(),
        Boolean(r1Drop) && r1Drop.getPath(),
        Boolean(r2Pick) && r2Pick.getPath(),
        Boolean(r2Drop) && r2Drop.getPath(),
        'lit:',
        litIn,
        'rmParent:',
        rmParent,
    ...
    const insComponent = (
      pick2Slot == null ? insHere && !rmParent && !rmHere : litOut !== undefined
    )
      ? wd.getComponent()
      : null;
    ...
    if (insComponent)
      ...
      log(
        'no insComponent - this insert will be discarded',
        insHere && !rmParent && !rmHere && pick2Slot == null,
        pick2Slot != null && litOut !== undefined,
      );
    ...
    const p2DropAdv = advancer(
      r2Drop,
      (k, c) => (hasDrop(c) ? p2DropOff - k - 1 : k - p2DropOff),
      (k, c) => {
        if (hasDrop(c)) p2DropOff++;
      },
    );

    const p1PickAdv = advancer(
      r1Pick,
      (k, c) => (hasPick(c) ? p1PickOff - k - 1 : k - p1PickOff),
      (k, c) => {
        if (hasPick(c)) p1PickOff++;
      },
    );
    ...
    eachChildOf(r1Drop, r2Pick, (inKey, _p1Drop, _p2Pick) => {
      ...
      if (typeof inKey === 'number') {
        const p2Mid = inKey + p2PickOff;
        _p2Drop = p2DropAdv(p2Mid);
        p2DropKey = p2Mid + p2DropOff;

        const p1Mid = inKey + p1DropOff;
      ...
      const _lit =
        hasContainerLiteral && !hasDrop(getComponent(_p1Drop))
          ? (litOut as any)[litKey]
          : undefined;

      log('_lit', _lit, litOut, litKey);
      const _litResult = xfBoundary(
        _p1Pick,
        _p1Drop,
        _p2Pick,
        _p2Drop,
      ...
      if (hasContainerLiteral && !rmChildren) {
        ...
        if (_lit !== _litResult) {
          if (!isCloned) {
            litOut = Array.isArray(litOut) ? litOut.slice() : { ...litOut };
            isCloned = true;
          }
  ...
  log(
    'heldPickWrites',
    heldPickWrites.map((w) => w.get()),
  );
  log(
    'heldDropWrites',
    heldDropWrites.map((w) => w.get()),
  );
...
function invert(op: JSONOp): JSONOp {
  ...
  function invertSimple(
    r: ReadCursor,
    w: WriteCursor,
    subDoc: Doc | undefined,
  ) {
    if (!RELEASE_MODE) log('invertSimple', r.getPath(), subDoc);
    incPrefix();
    const c = r.getComponent();
    let insertHere;
    let subdocModified = false;
  ...
  if (editsToTransform) {
    ...
    function transformEdits(
      rPick: ReadCursor | null,
      rDrop: ReadCursor,
      w: WriteCursor,
    ) {
      if (!RELEASE_MODE)
        log('invXE', rPick?.getPath(), rDrop.getPath(), w.getPath());
      incPrefix();
...
function makeInvertible(op: JSONOp, doc: Doc) {
  ...
  if (
    ...
    !anyComponent(
      op,
      (c) => c.r !== undefined || getEditType(c)?.makeInvertible != null,
    )
  )
    return op;
  ...
  const traversePick = (
    ...
    if (c) {
      ...
      if (c.r !== undefined) {
        if (subDoc === undefined)
          throw Error(
            'Invalid doc / op in makeInvertible: removed item missing from doc',
          );
    ...
    for (const key of r) {
      ...
      const keyRaw = typeof key === 'number' ? key - listOff : key;
...
function tryTransform(
  ...
  if (!RELEASE_MODE && debugMode) {
    ...
    log(
      `repro: transform(${[op1, op2, direction]
  ...
  function scanOp2Pick(
    r2Pick: ReadCursor,
    r1Pick: ReadCursor | null = null,
    removed1: ReadCursor | null,
  ) {
    ...
    if (!RELEASE_MODE)
      log(
        'scanOp2Pick',
        'r1Pick',
        r1Pick && r1Pick.getPath(),
        'r2Pick',
        r2Pick.getPath(),
        'removed1:',
        removed1 && removed1.getPath(),
      );
    incPrefix();
  ...
  if (!RELEASE_MODE) {
    ...
    log(
      'held op2 pick',
      heldOp2PickByOp1.map((x) => Boolean(x)),
    );
  }
  ...
  function scanOp2Drop(
    ...
    r2Pick: ReadCursor | null,
    r2Drop: ReadCursor,
    pickSlot1: number | null,
    removed1: ReadCursor | null,
  ) {
    // r1Pick, r2Pick could be null.
    if (!RELEASE_MODE)
      log(
        'scanOp2Drop',
        'r1Pick',
        r1Pick && r1Pick.getPath(),
        r2Pick && r2Pick.getPath(),
        'r2Drop',
        r2Drop.getPath(),
        'pickSlot1',
        pickSlot1,
    ...
    if (c2d) {
      if ((slot2 = c2d.d) != null) {
        ...
        // Teleport r2Pick
        log('tele r2Pick to op2 slot2', slot2, Boolean(cancelledOp2[slot2]));
        ...
        if (cancelledOp2[slot2]) {
          ...
          removed1 &&
          (side === RIGHT || op1PickAtOp2Pick[slot2] == null)
        ) {
  ...
  log(
    'held op2 drop',
    heldOp2DropByOp2.map((x) => x && x.get()),
  );
  if (conflict) {
  ...
  log(
    'discarded op2 drop',
    discardedOp2Drop.map((x) => Boolean(x)),
  );
  ...
  function writeOp1Pick(
    ...
    if (!RELEASE_MODE)
      log(
        ...
        r2Pick && r2Pick.getPath(),
        'r2Drop',
        r2Drop && r2Drop.getPath(),
        'w',
        w.getPath(),
        'removed2',
        removed2 && removed2.getPath(),
      );
    incPrefix();
    ...
    if (hasPick(c2p)) {
      ...
      if (slot2 != null) {
        ...
        if (!RELEASE_MODE)
          log(
            'teleporting via op2 slot2',
            slot2,
            'to',
            heldOp2DropByOp2[slot2]!.getPath(),
          );
        ...
        // op2 is a remove (c2p.r !== undefined)
    ...
    if (c1) {
      ...
      if (slot1 != null) {
        ...
        if (removed2 || iAmMoved) {
          log('Cancelling remove', c1, Boolean(removed2), iAmMoved);
    ...
    log('children', '2p', Boolean(r2Pick), '2d', Boolean(r2Drop));
    if (r1Pick)
      for (const key of r1Pick) {
        ...
        if (typeof key === 'string') {
          ...
          const p2Drop_ = hasPick(getComponent(p2Pick_))
            ? null
            : ap2Drop(p2Mid);
  ...
  if (!RELEASE_MODE) {
    ...
    log(
      'held picks',
      heldOp1PickByOp1.map((x) => x.getComponent() || Boolean(x)),
    );
    log(
      'held op2 drops by op1',
      heldOp2DropByOp1.map((x) => x.getComponent() || Boolean(x)),
    );
    log(
      'held pick writes',
      heldPickWrites.map((w) => w.get()),
  ...
  function writeOp1Drop(
    p1Pick: ReadCursor | null,
    p1Drop: ReadCursor,
    p2Pick: ReadCursor | null,
    p2Drop: ReadCursor | null,
    w: WriteCursor,
    removed2: ReadCursor | null /*, insidePick2*/,
  ) {
    assert(p1Drop);
    ...
    if (!RELEASE_MODE)
      log(
        'drop',
        p1Pick && p1Pick.getPath(),
        p1Drop.getPath(),
        p2Pick && p2Pick.getPath(),
        p2Drop && p2Drop.getPath(),
        'c:',
        c1d,
        'r:',
        Boolean(removed2) /*, 'ip2', insidePick2*/,
      );
    incPrefix();
    ...
    if (hasDrop(c1d)) {
      ...
      if (c1d!.i !== undefined || (slot1 != null && pc)) {
        ...
        if (
          c2d &&
          (c2d.i !== undefined ||
            ((slot2 = c2d.d) != null && !cancelledOp2[slot2]))
        ) {
          identical =
            slot2 != null
          ...
          if (
            !identical &&
            (slot2 == null || side === RIGHT || op1PickAtOp2Pick[slot2] == null)
          ) {
            ...
            if (conflict == null)
              conflict = {
                ...
                op1: insOrMv(
                  slot1 != null ? heldOp1PickByOp1[slot1] : null,
                  p1Drop,
                  c1d!,
                ),
                op2: insOrMv(
                  slot2 != null ? heldOp2PickByOp2[slot2]! : null,
                  p2Drop!,
                  c2d,
                ),
              };
        ...
        log('write', 'identical', identical, 'removed2', Boolean(removed2));
    ...
    if (hasPick(c2p)) {
      ...
      if (
        (c2p!.r !== undefined && (!c1p || c1p.r === undefined)) ||
        cancelledOp2[slot2!]
      ) {
        ...
        if (side === RIGHT || op1PickAtOp2Pick[slot2] == null) {
          ...
          if (!(w = heldDropWrites[slot2]))
            w = heldDropWrites[slot2] = writeCursor();
          w.reset();
          removed2 = null;
        }
      }
    ...
    if (t1) {
      log('edit:', Boolean(removed2), c1d, c2d);
      const e1 = getEdit(c1d!);
      if (!removed2) {
        const t2 = getEditType(c2d);
        let e;
    ...
    for (const key of p1Drop) {
      ...
      if (typeof key === 'number') {
...

        {
          ...
          while (p1pValid && typeof (p1k = p1Pick!.getKey()) === 'number') {
            ...
            if (hp) {
              ...
              const slot1 = c.p;
              log(
                'considering outPickOff--',
                c,
                cancelledRemoves,
                pickComponents[slot1!],
                op1PickAtOp2Pick.includes(slot1!),
              );
              log(
                c.d != null,
              ...
              if (
                ...
                (slot1 != null &&
                  pickComponents[slot1] &&
                  (side === RIGHT || !op1PickAtOp2Pick.includes(slot1)))
              ) {
          ...
          while (p2dValid && typeof (p2dk = p2Drop!.getKey()) === 'number') {
            ...
            if (hd2) {
              ...
              log(
                ...
                slot2,
                cancelledOp2[slot2],
                op1PickAtOp2Pick[slot2],
              );
              if (
                c.i !== undefined ||
                (!cancelledOp2[slot2] &&
                  (op1PickAtOp2Pick[slot2] == null || side === RIGHT))
              ) {
        ...
        let p1k;
        while (p1pValid) {
          // Skip items in p1Pick until key
          p1k = p1Pick!.getKey();
  ...
  const eachDrop = <W extends null | WriteCursor>(
    r: ReadCursor,
    w: W,
    fn: (slot: number, r: ReadCursor, w: W) => void,
  ) =>
    r.traverse(w, (c, w) => {
  ...
  function writeHeldOp2Drop(
    p2Drop: ReadCursor,
    outPick: ReadCursor | null,
    outDrop: ReadCursor | null,
    w: WriteCursor,
    parentC: JSONOpComponent | null,
    removedOut: Doc,
  ) {
    ...
    if (!RELEASE_MODE)
      log(
        ...
        p2Drop.getPath(),
        'outPick',
        outPick && outPick.getPath(),
        'outDrop',
        outDrop && outDrop.getPath(),
        'pc',
        parentC,
        'rm out',
        removedOut,
    ...
    if (coutp && hasPick(coutp)) {
      if (coutp.p != null) {
        ...
        const slot = coutp.p;
        log(
          'teleporting writes to output slot',
          slot,
          heldOutDropRead[slot].getPath(),
        );
    ...
    const oDropAdv = advancer(
      outDrop,
      (k, c) => (hasDrop(c) ? -(k - outDropOff) - 1 : k - outDropOff),
      (k, c) => {
        if (hasDrop(c)) {
          outDropOff++;
          log('outDropOff++');
  ...
  if ((heldDropWrites.length || cancelledOp2.length) && !conflict) {
    ...
    heldDropWrites.forEach((hdw) => {
      if (hdw)
        eachDrop(readCursor(hdw.get()), null, (slotOut, r) => {
          heldOutDropRead[slotOut] = r.clone();
        });
    });
    ...
    if (!RELEASE_MODE) {
      ...
      log(
        'heldOutDropRead',
        heldOutDropRead.map((r) => r && r.getPath()),
      );
    }
    ...
    if (!RELEASE_MODE)
      log(
        'held output writes',
        heldOutDropWrites.map((h) => h.get()),
      );
    if (heldOutDropWrites.length) {
      const heldOutDropContent = heldOutDropWrites.map((w) =>
        w ? w.get() : null,
      );
        ...
        heldOutDropContent.forEach((data, slotOut) => {
          if (data != null) {
            eachDrop(readCursor(data), null, (c) => {
              ...
              w1.writeMove(
                heldOp1PickByOp1[slot1].getPath(),
              ...
              if (slot2s)
                slot2s.forEach((slot2) => {
                  if (
                    ...
                    w2.writeMove(
                      heldOp2PickByOp2[slot2]!.getPath(),
                      heldOp2DropByOp2[slot2]!.getPath(),
                      nextSlot2++,
                    );
                  }
...
const resolveConflict = (conflict: Conflict, side: 'left' | 'right') => {
  ...
  switch (type) {
    ...
    case ConflictType.RM_UNEXPECTED_CONTENT:
      ...
      readCursor(op1).traverse(null, (c) => {
        if (c.r !== undefined) op1HasRemove = true;
      });
      return op1HasRemove
        ? [null, opThatRemovesDE(op2)]
        : [opThatRemovesDE(op1), null];
...
const invConflict = ({ type, op1, op2 }: Conflict) => ({
  type,
  op1: op2,
  op2: op1,
});
const normalizeConflict = ({ type, op1, op2 }: Conflict) => ({
  type,
  op1: normalize(op1),
  op2: normalize(op2),
});
...
function transformWithConflictsPred(
  allowConflict: AllowConflictPred,
  op1: JSONOp,
  op2: JSONOp,
  side: 'left' | 'right',
) {
    ...
    if (res.ok) return compose(r2Aggregate, res.result);
      ...
      if (!RELEASE_MODE && conflict.type === ConflictType.BLACKHOLE) {
        ...
        try {
          assert(
            deepEqual(
              normalizeConflict(res2.conflict) as any,
              normalizeConflict(invConflict(conflict)) as any,
            ),
          );
        } catch (e) {
