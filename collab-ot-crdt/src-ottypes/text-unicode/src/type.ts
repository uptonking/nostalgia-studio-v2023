...
export function eachOp(
  op: TextOp,
  fn: (c: TextOpComponent, prePos: number, postPos: number) => void,
) {
  let prePos = 0;
  let postPos = 0;
...
function mapOp(
  op: TextOp,
  fn: (c: TextOpComponent, prePos: number, postPos: number) => TextOpComponent,
): TextOp {
  const newOp: TextOp = [];
...
export const dlen = (d: number | string) =>
  typeof d === 'number' ? d : strPosToUni(d);
  ...
  const take = (n: number): TextOpComponent | null => {
    ...
    if (typeof c === 'number') {
      ...
      if (n === -1 || !isPre || dlen(c.d) - offset <= n) {
        ...
        const result = dslice(c.d, offset, offset + n);
        offset += n;
        return { d: result };
      }
    }
...
function transform(op1: TextOp, op2: TextOp, side: 'left' | 'right') {
  ...
  if (
    ...
    throw Error(
      'Internal error - composed operation should also be invertible',
    );
  }
    ...
    switch (typeof c) {
      case 'object': // Delete
        if (typeof c.d === 'number') {
          throw Error(
            'Cannot invert text op: Deleted characters missing from operation. makeInvertible must be called first.',
          );
        } else return c.d; // delete -> insert

      case 'string':
        return { d: c }; // Insert -> delete
      case 'number':
        return c; // skip -> skip
    }
  });
