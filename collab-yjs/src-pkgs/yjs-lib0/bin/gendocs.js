...
jsdoc
  ...
  .then((json) => {
    ...
    for (const fileName in modules) {
      ...
      strBuilder.push(
        `<pre>import * as ${mod.name} from 'lib0/${fileName.slice(
          0,
          -3,
        )}'</pre>`,
      );
          ...
          switch (item.kind) {
            ...
            case 'constant': {
              if (item.params == null && item.returns == null) {
                ...
                strBuilder.push(
                  `<b><code>${item.longname.slice(7)}${
                    typeEval ? ': ' + toSafeHtml(typeEval[1]) : ''
                  }</code></b><br>`,
                );
                if (item.description) {
            ...
            case 'function': {
              ...
              const paramVal = (item.params || [])
                .map(
                  (ret) =>
                    `${ret.name}: ${
                      getOriginalParamTypeDecl(ret.name) ||
                      ret.type.names.join('|')
                    }`,
                )
                .join(', ');
              ...
              strBuilder.push(
                `<b><code>${
                  item.kind === 'class' ? 'new ' : ''
                }${item.longname.slice(7)}(${toSafeHtml(paramVal)})${toSafeHtml(
                  returnVal,
                )}</code></b><br>`,
              );
              const desc =
            ...
            case 'member': {
              if (item.type) {
                strBuilder.push(
                  `<b><code>${item.longname.slice(7)}: ${toSafeHtml(
                    jsdocTypeRegex.exec(item.comment)[1],
                  )}</code></b><br>`,
                );
                if (item.description) {
                  strBuilder.push(`<dd>${item.description}</dd>`);
