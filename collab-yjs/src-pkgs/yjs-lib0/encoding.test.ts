...
function test(testname, write, read, val, doLog = true) {
  ...
  const utf8ByteLength = string.utf8ByteLength(String(val));
...
export const testStringEncodingPerformanceNativeVsPolyfill = () => {
  ...
  t.assert(
    nativeTimeSmall < customTimeSmall * 2,
...
export const testDecodingPerformanceNativeVsPolyfill = () => {
  ...
  t.assert(
    nativeTimeSmall < polyfillTimeSmall * 1.5,
    'Small dataset: We expect native decoding to be not much worse than',
  );
  t.assert(
    nativeTimeLarge < polyfillTimeLarge,
...
export const testLargeNumberAnyEncoding = (tc) => {
  const encoder = encoding.createEncoder();
  const num = -2.2062063918362897e50;
