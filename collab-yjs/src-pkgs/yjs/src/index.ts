...
const getItemCleanStart = (transaction, id) => {
  const structs = /** @type {Array<Item>} */ transaction.doc.store.clients.get(
    id.client,
  );
...
class UndoManager extends Observable {
  ...
  constructor(
    typeScope,
    {
      ...
      doc = /** @type {Doc} */ array.isArray(typeScope)
        ? typeScope[0].doc
        : typeScope.doc,
    } = {},
  ) {
...
const deleteText = (transaction, currPos, length) => {
  ...
  const parent = /** @type {AbstractType<any>} */ /** @type {Item} */ (
    currPos.left || currPos.right
  ).parent;
  if (parent._searchMarker) {
    updateMarkerChanges(
      parent._searchMarker,
