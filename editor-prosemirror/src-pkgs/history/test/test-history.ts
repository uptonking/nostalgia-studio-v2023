...
describe('history', () => {
  ...
  it('can go back and forth through history multiple times', () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      ist(
        state.doc,
        re ? doc(p('top'), p('zero one two three')) : doc(p()),
        eq,
      );
    }
  });
  ...
  it('rebases selection on undo', () => {
    ...
    state = state.apply(
      state.tr.insert(1, schema.text('---')).setMeta('addToHistory', false),
    );
    state = command(state, undo);
  ...
  it('includes transactions appended to undo in the redo history', () => {
    ...
    state = state.apply(
      state.tr.insert(2, schema.text('I')).setMeta('add', 'A'),
    );
    ist(state.doc, doc(p('AxI')), eq);
  ...
  it("doesn't close the history on appended transactions", () => {
    ...
    state = state.apply(
      state.tr.insert(2, schema.text('R')).setMeta('add', 'A'),
    );
  ...
  it('supports rebasing', () => {
    ...
    //       \
    //        - right
    const rightStep = new ReplaceStep(
      5,
      5,
      new Slice(Fragment.from(schema.text(' right')), 0, 0),
    );
    state = state.apply(state.tr.step(rightStep));
    ist(state.doc, doc(p('base right')), eq);
    ist(undoDepth(state), 2);
    const leftStep = new ReplaceStep(
      1,
      1,
      new Slice(Fragment.from(schema.text('left ')), 0, 0),
    );
