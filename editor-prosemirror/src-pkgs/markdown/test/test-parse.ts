...
describe('markdown', () => {
  ...
  it('parses a code block', () =>
    same(
      ...
      doc(
        p('Some code:'),
        schema.node('code_block', { params: '' }, [schema.text('Here it is')]),
        p('Para'),
      ),
    ));
  ...
  it('parses a fenced code block with info string', () =>
    same(
      ...
      doc(
        p('foo'),
        schema.node('code_block', { params: 'javascript' }, [schema.text('1')]),
      ),
    ));
  ...
  it('parses links inside strong text', () =>
    same('**[link](foo) is bold**', doc(p(strong(a('link'), ' is bold')))));
  ...
  it('parses urls', () =>
    same(
      ...
      doc(
        p(
          'Link to ',
          link({ href: 'https://prosemirror.net' }, 'https://prosemirror.net'),
        ),
      ),
    ));
  ...
  it('parses an image', () =>
    same("Here's an image: ![x](img.png)", doc(p("Here's an image: ", img()))));
  ...
  it('ignores HTML tags', () =>
    same('Foo < img> bar', doc(p('Foo < img> bar'))));

  it("doesn't accidentally generate list markup", () =>
    same('1\\. foo', doc(p('1. foo'))));
