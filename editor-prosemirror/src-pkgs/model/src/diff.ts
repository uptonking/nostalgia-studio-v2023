...
export function findDiffStart(a: Fragment, b: Fragment, pos: number): number | null {
  for (let i = 0;; i++) {
    ...
    const childA = a.child(i); const childB = b.child(i)
...
export function findDiffEnd(a: Fragment, b: Fragment, posA: number, posB: number): {a: number, b: number} | null {
  for (let iA = a.childCount, iB = b.childCount;;) {
    ...
    const childA = a.child(--iA); const childB = b.child(--iB); const size = childA.nodeSize
    if (childA == childB) {
      posA -= size; posB -= size
      continue
    }
    ...
    if (childA.isText && childA.text != childB.text) {
      let same = 0; const minSize = Math.min(childA.text!.length, childB.text!.length)
