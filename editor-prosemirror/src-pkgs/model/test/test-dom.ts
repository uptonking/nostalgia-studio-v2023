...
const xmlDocument = new JSDOM('<tag/>', { contentType: 'application/xml' })
  .window.document;
...
describe('DOMParser', () => {
  describe('parse', () => {
    ...
    function test(doc: PMNode, html: string, document_ = document) {
      return () => {
        const derivedDOM = document_.createElement('div');
    ...
    it(
      'can represent and unordered list',
      test(
        doc(
    ...
    it(
      ...
      test(doc(p(em('hi', br(), 'x'))), '<p><em>hi<br>x</em></p>'),
    );

    it(
      "doesn't collapse non-breaking spaces",
      test(
        doc(p('\u00a0 \u00a0hello\u00a0')),
        '<p>\u00a0 \u00a0hello\u00a0</p>',
      ),
    ...
    it('can parse marks on block nodes', () => {
      const commentSchema = new Schema({
        nodes: schema.spec.nodes.update('doc', {
          marks: 'comment',
          ...schema.spec.nodes.get('doc'),
        }),
        marks: schema.spec.marks.update('comment', {
          parseDOM: [{ tag: 'div.comment' }],
          toDOM() {
            return ['div', { class: 'comment' }, 0];
          },
        }),
      });
      ...
      test(
        b.doc(
          b.paragraph('one'),
          b.comment(b.paragraph('two'), b.paragraph(b.strong('three'))),
          b.paragraph('four'),
        ),
    ...
    it('parses unique, non-exclusive, same-typed marks', () => {
      const commentSchema = new Schema({
        ...
        marks: schema.spec.marks.update('comment', {
          ...
          parseDOM: [
            {
              ...
              getAttrs(dom) {
                return {
                  id: parseInt(
                    (dom as HTMLElement).getAttribute('data-id')!,
                    10,
                  ),
                };
              },
          ...
          excludes: '',
          toDOM(mark: Mark) {
            return ['span', { class: 'comment', 'data-id': mark.attrs.id }, 0];
          },
        }),
      });
      const b = builders(commentSchema);
      test(
        b.schema.nodes.doc.createAndFill(undefined, [
    ...
    it('serializes non-spanning marks correctly', () => {
      ...
      test(
        b.doc(b.paragraph(b.test('a', b.image({ src: 'x' }), 'b'))),
    ...
    it('serializes an element and an attribute with XML namespace', () => {
          ...
          svg: {
            ...
            toDOM() {
              return [
                ...
                ['use', { 'http://www.w3.org/1999/xlink href': '#svg-id' }],
              ];
            },
          },
        },
      ...
      dom.appendChild(
        DOMSerializer.fromSchema(xmlnsSchema).serializeFragment(d.content, {
      ...
      ist(
        dom.querySelector('use').attributes[0].namespaceURI,
    ...
    it(
      ...
      recover('<ol><p>Oh no</p></ol>', doc(ol(li(p('Oh no'))))),
    );

    it(
      'wraps a list item in a list',
      recover('<li>hey</li>', doc(ol(li(p('hey'))))),
    );

    it(
      'can turn divs into paragraphs',
      recover('<div>hi</div><div>bye</div>', doc(p('hi'), p('bye'))),
    );
    ...
    it('wraps stray text in a paragraph', recover('hi', doc(p('hi'))));

    it(
      'ignores an extra wrapping <div>',
      recover('<div><p>one</p><p>two</p></div>', doc(p('one'), p('two'))),
    );
    ...
    it(
      ...
      recover('<pre>foo<br>bar</pre>', doc(pre('foo\nbar'))),
    );
    ...
    it(
      ...
      recover('<div>hello<hr/>bye</div>', doc(p('hello'), hr(), p('bye'))),
    );
    ...
    it(
      ...
      recover('<p> <b>&#09;</b></p>', doc(p())),
    );

    it(
      'normalizes random spaces',
      recover('<p><b>1 </b>  </p>', doc(p(strong('1')))),
    );

    it('can parse an empty code block', recover('<pre></pre>', doc(pre())));
    ...
    it(
      'normalizes newlines when preserving whitespace',
      recover('<p>foo  bar\nbaz</p>', doc(p('foo  bar baz')), {
    ...
    it(
      'ignores <script> tags',
      recover("<p>hello<script>alert('x')</script>!</p>", doc(p('hello!'))),
    );

    it(
      'can handle a head/body input structure',
      recover(
    ...
    it(
      'interprets font-style: italic as em',
      recover(
    ...
    it(
      'interprets font-weight: bold as strong',
      recover(
    ...
    it(
      'ignores unknown inline tags',
      recover('<p><u>a</u>bc</p>', doc(p('abc'))),
    );
    ...
    it(
      "keeps applying a mark for the all of the node's content",
      recover(
    ...
    it(
      "doesn't ignore whitespace-only nodes in preserveWhitespace full mode",
    ...
    it(
      'accepts the topNode option',
      parse(
        '<li>wow</li><li>such</li>',
        { topNode: schema.nodes.bullet_list.createAndFill()! },
    ...
    it(
      'accepts the topMatch option',
      parse(
        '<ul><li>x</li></ul>',
        { topNode: item, topMatch: item.contentMatchAt(1)! },
        li(ul(li(p('x')))),
      ),
    );
    ...
    it(
      'accepts the preserveWhitespace option',
      parse('foo   bar', { preserveWhitespace: true }, doc(p('foo   bar'))),
    );
    ...
    function open(
      ...
      return () => {
        ...
        ist(
          ...
          new Slice(
            Fragment.from(
              nodes.map((n) => (typeof n === 'string' ? schema.text(n) : n)),
            ),
            openStart,
            openEnd,
          ),
    ...
    it('can parse an open slice', open('foo', ['foo'], 0, 0));

    it(
      'will accept weird siblings',
      open('foo<p>bar</p>', ['foo', p('bar')], 0, 1),
    );

    it(
      'will open all the way to the inner nodes',
      open(
        '<ul><li>foo</li><li>bar<br></li></ul>',
        [ul(li(p('foo')), li(p('bar', br())))],
        3,
        3,
      ),
    );
    ...
    it(
      'accepts content open to the left',
      open('<li><ul><li>a</li></ul></li>', [li(ul(li(p('a'))))], 4, 4),
    );

    it(
      'accepts content open to the right',
      open('<li>foo</li><li></li>', [li(p('foo')), li()], 2, 1),
    );

    it(
      'will create textblocks for block nodes',
      open(
    ...
    it(
      'can parse marks at the start of defaulted textblocks',
      open(
    ...
    it(
      'will not apply invalid marks to nodes',
      open(
    ...
    it(
      'will apply pending marks from parents to all children',
      open(
        "<ul style='font-weight: bold'><li>foo</li><li>bar</li></ul>",
        [ul(li(p(strong('foo'))), li(p(strong('bar'))))],
        3,
        3,
      ),
    );
    ...
    it(
      'can parse nested mark with same type',
      open(
    ...
    it(
      'drops block-level whitespace',
      open('<div> </div>', [], 0, 0, { preserveWhitespace: true }),
    );

    it(
      'keeps whitespace in inline elements',
      open('<b> </b>', [p(strong(' ')).child(0)], 0, 0, {
        preserveWhitespace: true,
      }),
    );
    ...
    it('can parse nested mark with same type but different attrs', () => {
      ...
      ist(
        ...
        new Slice(
          Fragment.from(
            b.schema.nodes.paragraph.create(undefined, [
              b.schema.text('o', [
                b.schema.marks.s.create({ 'data-s': 'style' }),
              ]),
              b.schema.text('o', [
                b.schema.marks.s.create({ 'data-s': 'style' }),
                b.schema.marks.s.create({ 'data-s': 'tag' }),
              ]),
              b.schema.text('o', [
                b.schema.marks.s.create({ 'data-s': 'style' }),
              ]),
    ...
    function find(html: string, doc: PMNode) {
      return () => {
        const dom = document.createElement('div');
        dom.innerHTML = html;
        const tag = dom.querySelector('var');
    ...
    it(
      'can find a position at the start of a paragraph',
      find('<p><var></var>hello</p>', doc(p('<a>hello'))),
    );

    it(
      'can find a position at the end of a paragraph',
      find('<p>hello<var></var></p>', doc(p('hello<a>'))),
    );

    it(
      'can find a position inside text',
      find('<p>hel<var></var>lo</p>', doc(p('hel<a>lo'))),
    );

    it(
      'can find a position inside an ignored node',
      find(
    ...
    it(
      'can find a position between nodes',
      find(
    ...
    it(
      'can find a position at the start of the document',
      find('<var></var><p>hi</p>', doc('<a>', p('hi'))),
    );

    it(
      'can find a position at the end of the document',
    ...
    it(
      'uses a custom top node when parsing',
      test(
        quoteSchema.node(
          'blockquote',
          null,
          quoteSchema.node('paragraph', null, quoteSchema.text('hello')),
        ),
        '<p>hello</p>',
      ),
    );
    ...
    function contextParser(context: string) {
      return new DOMParser(
        schema,
        [{ tag: 'foo', node: 'horizontal_rule', context } as ParseRule].concat(
          DOMParser.schemaRules(schema) as ParseRule[],
        ),
      );
    }
    ...
    it('recognizes context restrictions', () => {
      ist(
        contextParser('blockquote/').parse(
          domFrom(
    ...
    it('understands nested context restrictions', () => {
      ist(
        contextParser('blockquote/ordered_list//').parse(
          domFrom(
            '<foo></foo><blockquote><foo></foo><ol><li><p>a</p><foo></foo></li></ol></blockquote>',
          ),
        ),
        doc(blockquote(ol(li(p('a'), hr())))),
        eq,
      );
    });
    ...
    it('understands double slashes in context restrictions', () => {
      ist(
        contextParser('blockquote//list_item/').parse(
          domFrom(
    ...
    it('understands pipes in context restrictions', () => {
      ist(
        contextParser('list_item/|blockquote/').parse(
          domFrom(
            '<foo></foo><blockquote><p></p><foo></foo></blockquote><ol><li><p>a</p><foo></foo></li></ol>',
          ),
        ),
        doc(blockquote(p(), hr()), ol(li(p('a'), hr()))),
        eq,
      );
    });
    ...
    it('uses the passed context', () => {
      ...
      ist(
        contextParser('doc//blockquote/').parse(
    ...
    it('uses the passed context when parsing a slice', () => {
      const cxDoc = doc(blockquote('<a>', hr()));
      ist(
        contextParser('doc//blockquote/').parseSlice(domFrom('<foo></foo>'), {
          context: cxDoc.resolve((cxDoc as any).tag.a),
        }),
        new Slice(blockquote(hr()).content, 0, 0),
        eq,
      );
    });
    ...
    it('can close parent nodes from a rule', () => {
      const closeParser = new DOMParser(
        schema,
        [{ tag: 'br', closeParent: true } as ParseRule].concat(
          DOMParser.schemaRules(schema),
        ),
      );
      ist(
        closeParser.parse(domFrom('<p>one<br>two</p>')),
        doc(p('one'), p('two')),
        eq,
      );
    });
    ...
    it('supports non-consuming node rules', () => {
      const parser = new DOMParser(
        schema,
        [
          { tag: 'ol', consuming: false, node: 'blockquote' } as ParseRule,
        ].concat(DOMParser.schemaRules(schema)),
      );
      ist(
        parser.parse(domFrom('<ol><p>one</p></ol>')),
        doc(blockquote(ol(li(p('one'))))),
        eq,
      );
    });
    ...
    it('supports non-consuming style rules', () => {
      const parser = new DOMParser(
        schema,
        [
          { style: 'font-weight', consuming: false, mark: 'em' } as ParseRule,
        ].concat(DOMParser.schemaRules(schema)),
      );
      ist(
        parser.parse(
          domFrom("<p><span style='font-weight: 800'>one</span></p>"),
        ),
        doc(p(em(strong('one')))),
        eq,
      );
    ...
    it(
      "doesn't get confused by nested mark tags",
      recover(
...
describe('DOMSerializer', () => {
  const noEm = new DOMSerializer(serializer.nodes, {
    ...serializer.marks,
    em: null,
  });
  ...
  it('can render marks with complex structure', () => {
    const deepEm = new DOMSerializer(serializer.nodes, {
      ...serializer.marks,
      em() {
        return ['em', ['i', { 'data-emphasis': true }, 0]];
      },
    });
    const node = deepEm.serializeNode(
