...
const remark1 = custom.remark.create({ id: 1 });
const remark2 = custom.remark.create({ id: 2 });
const user1 = custom.user.create({ id: 1 });
const user2 = custom.user.create({ id: 2 });
const customEm = custom.em.create();
const customStrong = custom.strong.create();
...
describe('Mark', () => {
  describe('sameSet', () => {
    ...
    it('recognizes identical links in set', () =>
      ist(
        Mark.sameSet([link('http://foo'), code], [link('http://foo'), code]),
      ));

    it('recognizes different links in set', () =>
      ist(
        !Mark.sameSet([link('http://foo'), code], [link('http://bar'), code]),
      ));
  });
  ...
  describe('addToSet', () => {
    ...
    it("doesn't add anything when another mark excludes the added mark", () =>
      ist(
        customEm.addToSet([remark1, customStrong]),
    ...
    it('remove excluded marks when adding a mark', () =>
      ist(
        customStrong.addToSet([remark1, customEm]),
        [remark1, customStrong],
        Mark.sameSet,
      ));
  });
  ...
  describe('removeFromSet', () => {
    ...
    it("doesn't remove a mark when its attrs differ", () =>
      ist(
        Mark.sameSet(
          link('http://foo', 'title').removeFromSet([link('http://foo')]),
          [link('http://foo')],
        ),
      ));
  });
