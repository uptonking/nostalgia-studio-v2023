...
describe('Node', () => {
  ...
  describe('cut', () => {
    ...
    it('works from the left', () =>
      cut(doc(blockquote(p('foo<b>bar'))), doc(blockquote(p('foo')))));

    it('works to the right', () =>
      cut(doc(blockquote(p('foo<a>bar'))), doc(blockquote(p('bar')))));
  ...
  describe('between', () => {
    function between(doc: Node, ...nodes: string[]) {
      ...
      doc.nodesBetween((doc as any).tag.a, (doc as any).tag.b, (node, pos) => {
        if (i == nodes.length)
          throw new Error(
            'More nodes iterated than listed (' + node.type.name + ')',
          );
        const compare = node.isText ? node.text! : node.type.name;
        if (compare != nodes[i++])
          throw new Error(
            'Expected ' +
              JSON.stringify(nodes[i - 1]) +
              ', got ' +
  ...
  describe('toString', () => {
    ...
    it('should have the default toString method [br]', () =>
      ist(br().toString(), 'hard_break'));
  ...
  describe('leafText', () => {
    it('should custom the textContent of a leaf node', () => {
      const contact = customSchema.nodes.contact.createChecked({
        name: 'Bob',
        email: 'bob@example.com',
      });
      const paragraph = customSchema.nodes.paragraph.createChecked({}, [
        customSchema.text('Hello '),
        contact,
      ]);
