...
const olDOM: DOMOutputSpec = ["ol", 0]; const ulDOM: DOMOutputSpec = ["ul", 0]; const liDOM: DOMOutputSpec = ["li", 0]
...
function doWrapInList(tr: Transaction, range: NodeRange, wrappers: {type: NodeType, attrs?: Attrs | null}[],
  ...
  let splitPos = range.start + wrappers.length - (joinBefore ? 2 : 0); const parent = range.parent
...
function liftToOuterList(state: EditorState, dispatch: (tr: Transaction) => void, itemType: NodeType, range: NodeRange) {
  const tr = state.tr; const end = range.end; const endOfList = range.$to.end(range.depth)
  if (end < endOfList) {
...
function liftOutOfList(state: EditorState, dispatch: (tr: Transaction) => void, range: NodeRange) {
  const tr = state.tr; const list = range.parent
  // Merge the list items into a single big item
  for (let pos = range.end, i = range.endIndex - 1, e = range.startIndex; i > e; i--) {
    pos -= list.child(i).nodeSize
    tr.delete(pos - 1, pos + 1)
  }
  const $start = tr.doc.resolve(range.start); const item = $start.nodeAfter!
  if (tr.mapping.map(range.end) != range.start + $start.nodeAfter!.nodeSize) return false
  const atStart = range.startIndex == 0; const atEnd = range.endIndex == list.childCount
  const parent = $start.node(-1); const indexBefore = $start.index(-1)
  if (!parent.canReplace(indexBefore + (atStart ? 0 : 1), indexBefore + 1,
                         item.content.append(atEnd ? Fragment.empty : Fragment.from(list))))
    return false
  const start = $start.pos; const end = start + item.nodeSize
...
export function sinkListItem(itemType: NodeType): Command {
  return function(state, dispatch) {
    ...
    const parent = range.parent; const nodeBefore = parent.child(startIndex - 1)
    ...
    if (dispatch) {
      ...
      const slice = new Slice(Fragment.from(itemType.create(null, Fragment.from(parent.type.create(null, inner)))),
                            nestedBefore ? 3 : 1, 0)
      const before = range.start; const after = range.end
