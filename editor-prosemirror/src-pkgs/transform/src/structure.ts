...
export function liftTarget(range: NodeRange): number | null {
  ...
  for (let depth = range.depth;; --depth) {
    ...
    const index = range.$from.index(depth); const endIndex = range.$to.indexAfter(depth)
...
export function lift(tr: Transform, range: NodeRange, target: number) {
  ...
  const gapStart = $from.before(depth + 1); const gapEnd = $to.after(depth + 1)
  let start = gapStart; let end = gapEnd

  let before = Fragment.empty; let openStart = 0
  ...
  let after = Fragment.empty; let openEnd = 0
  for (let d = depth, splitting = false; d > target; d--)
...
export function setBlockType(tr: Transform, from: number, to: number, type: NodeType, attrs: Attrs | null) {
  ...
  tr.doc.nodesBetween(from, to, (node, pos) => {
    if (node.isTextblock && !node.hasMarkup(type, attrs) && canChangeType(tr.doc, tr.mapping.slice(mapFrom).map(pos), type)) {
      ...
      const startM = mapping.map(pos, 1); const endM = mapping.map(pos + node.nodeSize, 1)
...
function canChangeType(doc: Node, pos: number, type: NodeType) {
  const $pos = doc.resolve(pos); const index = $pos.index()
...
export function canSplit(doc: Node, pos: number, depth = 1,
  ...
  const $pos = doc.resolve(pos); const base = $pos.depth - depth
  ...
  for (let d = $pos.depth - 1, i = depth - 2; d > base; d--, i--) {
    const node = $pos.node(d); const index = $pos.index(d)
...
export function split(tr: Transform, pos: number, depth = 1, typesAfter?: (null | {type: NodeType, attrs?: Attrs | null})[]) {
  const $pos = tr.doc.resolve(pos); let before = Fragment.empty; let after = Fragment.empty
...
export function joinPoint(doc: Node, pos: number, dir = -1) {
  ...
  for (let d = $pos.depth;; d--) {
    let before; let after; let index = $pos.index(d)
    if (d == $pos.depth) {
...
export function dropPoint(doc: Node, pos: number, slice: Slice): number | null {
  ...
  for (let pass = 1; pass <= (slice.openStart == 0 && slice.size ? 2 : 1); pass++) {
    for (let d = $pos.depth; d >= 0; d--) {
      ...
      const parent = $pos.node(d); let fits: boolean | null = false
      if (pass == 1) {
