...
function range(pos: number, end?: number) {
  return doc
    .resolve(pos)
    .blockRange(end == null ? undefined : doc.resolve(end));
}
    ...
    return () =>
      ist(
        canSplit(
          ...
          pos,
          depth,
          after == null ? undefined : [{ type: schema.nodes[after] }],
        ),
      );
  }
...
describe('Transform', () => {
  describe('replace', () => {
    function repl(
      ...
      return () => {
        const slice = content
          ? new Slice(content.content, openStart, openEnd)
          : Slice.empty;
    ...
    it(
      ...
      repl(
        n(
          'doc',
          n('sect', n('head'), n('figure', n('caption'), n('figureimage'))),
        ),
        7,
        9,
        n('doc', n('para', t('hi'))),
