...
describe('Transform', () => {
  describe('addMark', () => {
    function add(doc: Node, mark: Mark, expect: Node) {
      testTransform(
        new Transform(doc).addMark(tag(doc, 'a'), tag(doc, 'b'), mark),
        expect,
      );
    }
  ...
  describe('removeMark', () => {
    function rem(doc: Node, mark: Mark | null, expect: Node) {
      testTransform(
        new Transform(doc).removeMark(tag(doc, 'a'), tag(doc, 'b'), mark),
        expect,
      );
    }
  ...
  describe('delete', () => {
    function del(doc: Node, expect: Node) {
      testTransform(
        new Transform(doc).delete(tag(doc, 'a'), tag(doc, 'b')),
        expect,
      );
    }
  ...
  describe('join', () => {
    ...
    it('can join textblocks', () =>
      join(doc(p('foo'), '<a>', p('bar')), doc(p('foo<a>bar'))));
  });
  ...
  describe('split', () => {
    function split(
      ...
      if (expect == 'fail')
        ist.throws(() =>
          new Transform(doc).split(tag(doc, 'a'), depth, typesAfter),
        );
      else
        testTransform(
          new Transform(doc).split(tag(doc, 'a'), depth, typesAfter),
  ...
  describe('lift', () => {
    function lift(doc: Node, expect: Node) {
      ...
      testTransform(
        new Transform(doc).lift(range!, liftTarget(range!)!),
        expect,
      );
    }
  ...
  describe('wrap', () => {
    function wrap(doc: Node, expect: Node, type: string, attrs?: Attrs) {
      const range = doc
        ...
        .blockRange(doc.resolve(tag(doc, 'b') || tag(doc, 'a')));
      testTransform(
        new Transform(doc).wrap(
          range!,
          findWrapping(range!, schema.nodes[type], attrs)!,
        ),
        expect,
      );
    }
  ...
  describe('setBlockType', () => {
    ...
    it('works after another step', () => {
      ...
      const tr = new Transform(d).delete((d as any).tag.x, (d as any).tag.y);
      const pos = tr.mapping.map((d as any).tag.a);
  ...
  describe('setNodeMarkup', () => {
    function markup(doc: Node, expect: Node, type: string, attrs?: Attrs) {
      testTransform(
        new Transform(doc).setNodeMarkup(
          tag(doc, 'a'),
          schema.nodes[type],
          attrs,
        ),
  ...
  describe('replace', () => {
    ...
    it('can delete text', () =>
      repl(doc(p('hell<a>o y<b>ou')), null, doc(p('hell<a><b>ou'))));
    ...
    it('preserves marks on open slice block nodes', () => {
      const tr = new Transform(
        ms.node('doc', null, [ms.node('paragraph', null, [ms.text('a')])]),
      );
      tr.replace(
        3,
        3,
    ...
    it('can unwrap a body after a placed node', () => {
      ...
      ist(
        ...
        hb.doc(
          hb.h('Head'),
          hb.b(hb.h('Head'), hb.p('Content'), hb.p('Content')),
        ),
        eq,
      );
    });
    ...
    it("can wrap a paragraph in a body, even when it's not the first node", () => {
      const tr = new Transform(
        hb.doc(hb.h('Head'), hb.b(hb.p('One'), hb.p('Two'))),
      );
    ...
    it('can split a fragment and place its children in different parents', () => {
      const tr = new Transform(
        hb.doc(hb.h('Head'), hb.b(hb.h('One'), hb.p('Two'))),
      );
    ...
    it("doesn't fail when pasting a half-open slice with a title and a code block into an empty title", () => {
      ...
      const tr = new Transform(
        s.node('doc', null, [s.node('title', null, [])]),
      );
      tr.replace(
        1,
        1,
    ...
    it('can handle replacing in nodes with fixed content', () => {
      ...
      ist(
        new Transform(doc).replace(from, to, doc.slice(from, to)).doc,
        doc,
        eq,
      );
    });
  ...
  describe('replaceRangeWith', () => {
    function repl(doc: Node, node: Node, expect: Node) {
      testTransform(
        new Transform(doc).replaceRangeWith(
          tag(doc, 'a'),
          tag(doc, 'b') || tag(doc, 'a'),
          node,
        ),
  ...
  describe('addNodeMark', () => {
    function add(doc: Node, mark: Mark, expect: Node) {
      testTransform(
        new Transform(doc).addNodeMark(tag(doc, 'a'), mark),
        expect,
      );
    }
  ...
  describe('removeNodeMark', () => {
    function rm(doc: Node, mark: Mark | MarkType, expect: Node) {
      testTransform(
        new Transform(doc).removeNodeMark(tag(doc, 'a'), mark),
        expect,
      );
    }
  ...
  describe('setNodeAttribute', () => {
    function set(doc: Node, attr: string, value: any, expect: Node) {
      testTransform(
        new Transform(doc).setNodeAttribute(tag(doc, 'a'), attr, value),
        expect,
      );
    }
