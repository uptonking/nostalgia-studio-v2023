...
describe("Clipboard interface", () => {
  it("copies only the node for a node selection", () => {
    ...
    const {dom} = serializeForClipboard(view, NodeSelection.create(d, (d as any).tag.a).content())
  ...
  it("can read external HTML", () => {
    const view = tempEditor(); const $p = view.state.doc.resolve(1)
  ...
  it("will call transformPastedHTML", () => {
    const view = tempEditor({transformPastedHTML(_) { return "abc" }})
  ...
  it("will call transformPastedText", () => {
    const view = tempEditor({transformPastedText(_) { return "abc" }})
  ...
  it("allows text parsing to be overridden with clipboardTextParser", () => {
    const view = tempEditor({clipboardTextParser(text) { return doc(p(text.toUpperCase())).slice(1, text.length + 1) }})
  ...
  it("preserves attributes", () => {
    ...
    const view = tempEditor({doc: d})
    const {dom, text} = serializeForClipboard(view, TextSelection.create(d, (d as any).tag.a, (d as any).tag.b).content())
  ...
  it("adds necessary wrappers for parsing", () => {
    const s = tableSchema()
    const doc = s.node("doc", null, [s.node("table", null, [s.node("tr", null, [
