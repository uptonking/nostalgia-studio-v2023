...
function buildMap(
  doc: Node,
  ...decorations: (DecoSpec | ((tr: Transform) => Transform))[]
) {
...
describe('DecorationSet', () => {
  ...
  describe('find', () => {
    ...
    it('returns the correct offset for deeply nested decorations', () => {
      const set = build(doc(blockquote(blockquote(p('a')))), {
        from: 3,
        to: 4,
      });
  ...
  describe('map', () => {
    ...
    it('drops deleted decorations', () => {
      const { set } = buildMap(doc(p('foobar')), { from: 2, to: 3 }, (tr) =>
        tr.delete(1, 4),
      );
      ist(str(set), '[]');
    });
    ...
    it("isn't inclusive by default", () => {
      const { set } = buildMap(doc(p('foo')), { from: 2, to: 3 }, (tr) =>
        tr
          .replaceWith(2, 2, schema.text('.'))
          .replaceWith(4, 4, schema.text('?')),
      );
      ist(str(set), '[0: [2-3]]');
    });
    ...
    it('understands unclusiveLeft', () => {
      const { set } = buildMap(
        doc(p('foo')),
        { from: 2, to: 3, inclusiveStart: true },
        (tr) =>
          tr
            .replaceWith(2, 2, schema.text('.'))
            .replaceWith(4, 4, schema.text('?')),
    ...
    it('preserves subtrees not touched by mapping', () => {
      ...
      ist(
        (set as any).children[5].children[2],
        (oldSet as any).children[5].children[5],
      );
    });
    ...
    it("doesn't doubly map decorations nested in multiple nodes", () => {
      ...
      const tr = new Transform(d).replaceWith(
        0,
        3,
    ...
    it('can map the content of nodes that moved in the same transaction', () => {
      ...
      const tr = new Transform(d).step(
        new ReplaceAroundStep(0, 7, 2, 5, Slice.empty, 0, true),
      );
    ...
    it('maps inline decorations through ranges with > 3 elements', () => {
      ...
      di.resolve(di.tag.start)
        ...
        .forEach((node, offset, index) => {
          ...
          if (index > 0)
            ranges.push(
              posBeforeFirstWord + offset - 1,
              1,
              0,
              posBeforeFirstWord + offset,
              1,
              0,
            );
        });
  ...
  describe('add', () => {
    it('can add a local decoration', () => {
      ist(
        str(buildAdd(doc(p('foo'), p('bar')), { pos: 0 }, { pos: 5 })),
        '[0-0, 5-5]',
      );
    });
    ...
    it('can add a decoration in a new child', () => {
      ist(
        str(buildAdd(doc(p('foo'), p('bar')), { pos: 0 }, { pos: 3 })),
        '[0-0, 0: [2-2]]',
      );
    });
    ...
    it('can add a decoration to an existing child', () => {
      ist(
        str(buildAdd(doc(p('foo'), p('bar')), { pos: 1 }, { pos: 3 })),
        '[0: [0-0, 2-2]]',
      );
    });
    ...
    it('can add a decoration beyond an existing child', () => {
      ist(
        str(buildAdd(doc(blockquote(p('foo'))), { pos: 1 }, { pos: 4 })),
        '[0: [0-0, 0: [2-2]]]',
      );
    });
...
describe('removeOverlap', () => {
  ...
  it('correctly splits overlapping inline decorations', () => {
    ...
    ist(
      arrayStr(DecorationSet.removeOverlap(decs)),
      '0-1, 1-3, 1-3, 3-4, 3-4, 3-4, 4-5, 4-5, 5-6',
    );
  });
});
