...
describe("DOM change", () => {
  ...
  it("does not repaint a text node when it's typed into", () => {
    ...
    let mutated = false; const observer = new MutationObserver(() => mutated = true)
  ...
  it("recognizes a mark change as such", () => {
    const view = tempEditor({doc: doc(p("one")), dispatchTransaction(tr) {
  ...
  it("doesn't redraw content with marks when typing in front", () => {
    ...
    const bar = findTextNode(view.dom, "bar")!; const foo = findTextNode(view.dom, "foo")!
    foo.nodeValue = "froo"
    flush(view)
  ...
  it("doesn't confuse backspace with delete", () => {
    let steps: Step[]; const view = tempEditor({
      doc: doc(p("a<a>a")),
