...
function getSel() {
  const sel = window.getSelection()!
  let node = sel.focusNode; let offset = sel.focusOffset
...
describe("EditorView", () => {
  ...
  it("produces sensible screen coordinates in corner cases", () => {
    const view = tempEditor({doc: doc(p("one", em("two", strong("three"), img), br(), code("foo")), p())})
  ...
  it("returns correct screen coordinates for wrapped lines", () => {
    ...
    const top = view.coordsAtPos(1); let pos = 1; let end: {left: number, top: number, right: number, bottom: number} | undefined
    for (let i = 0; i < 100; i++) {
  ...
  it("updates the selection even if the DOM parameters look unchanged", () => {
    ...
    view.focus()
    const decos = DecorationSet.create(view.state.doc, [Decoration.inline(1, 4, {color: "green"})])
