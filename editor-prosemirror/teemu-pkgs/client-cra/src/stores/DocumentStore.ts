...
export class DocumentStore {
  ...
  @action deleteDocument = async (id: string) => {
    ...
    runInAction(() => {
      ...
      const doc =
        this.currentDocument?.doc ?? this.editorStore.createEmptyDoc();
      this.editorStore.setCurrentDoc(doc);
    });
  };
    ...
    if (wasThisUser) {
      ...
      const msg =
        visibility === 'global'
          ...
          : 'Document has been made private';
  ...
  @action syncDocument = () => {
    ...
    if (!this.currentDocument) {
      ...
      this.updateDocument(this.currentDocument.id, {
  ...
  @action toggleCollab = () => {
    if (this.currentDocument) {
      const visibility =
        this.currentDocument.visibility === 'private' ? 'global' : 'private';
