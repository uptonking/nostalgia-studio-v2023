...
export class ReactNodeView<P extends any = {}, A extends Attrs = {}>
  implements NodeView
{
  /**
  ...
  get dom() {
    if (!this._dom) {
      throw Error(
        'Accessing uninitialized dom inside ReactNodeView! Did you forget to add initialize method?',
      );
    }
    return this._dom;
  }
  ...
  init() {
    ...
    // expensive render?
    //
    this._dom = this.createContainerDOM();
  ...
  private renderReactComponent(component: React.ReactElement<any> | null) {
    ...
    this.ctx.portalProvider.render(component, this.dom!);
  }
  ...
  update(node: PMNode) {
    ...
    this.ctx.portalProvider.update(this.dom, this.createProps(node));
    return true;
  }
  ...
  destroy() {
    this.ctx.portalProvider.remove(this.dom);
    this._dom = undefined;
