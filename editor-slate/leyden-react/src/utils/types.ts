...
type KeptRenderElementProps = Omit<
  RenderSlateElementProps,
  'attributes' | 'element'
>;

export interface RenderElementProps<T extends ExternalElementType>
  extends KeptRenderElementProps {
  attributes: RenderElementAttributes;
  element: Element<T>;
}

export type ElementRenderer<T extends ExternalElementType> = FC<
...
export interface RenderCellProps<T extends CellType>
  extends Omit<RenderSlateElementProps, 'element'> {
  element: Cell<T>;
}
...
export interface RenderTextProps<T extends TextType>
