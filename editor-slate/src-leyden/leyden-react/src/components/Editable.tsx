...
export type Editable = Omit<
  Parameters<typeof SlateReactEditable>[0],
...
export const Editable: FC<Editable> = ({
  ...
  const handleKeyDown = useCallback<KeyboardEventHandler<HTMLDivElement>>(
    (e) => {
      ...
      if (e.key in cellNavigationKeys) {
        e.preventDefault();
        Transforms.moveCellSelection(editor, {
          direction: cellNavigationKeys[e.key],
        });
        return;
      }
    },
  ...
  return (
    <SlateReactEditable {...render} {...props} onKeyDown={handleKeyDown} />
  );
};
