...
type ExtendableExtraTypes = 'Validator';

type ExtendableTypes = ExtendableComponentTypes | ExtendableExtraTypes;

export const extendableComponentDefaultKey = 'EXTENDABLE_COMPONENT_DEFAULT_KEY';

interface WithCellType<T extends string> {
  cellType: T;
}
interface WithChildren<T extends Array<unknown>> {
  children: T;
}
interface WithData<T> {
  data: T;
}
interface WithDataOptional<T> {
  data?: T;
}
interface WithIsInline<T extends boolean> {
  isInline: T;
}
interface WithIsInlineOptional<T extends boolean> {
  isInline?: T;
}
interface WithIsVoid<T extends boolean> {
  isVoid: T;
}
interface WithIsVoidOptional<T extends boolean> {
  isVoid?: T;
}
interface WithText<T extends string> {
  text: T;
}
interface WithType<T extends string> {
  type: T;
}
interface WithValidator<T extends Validator> {
  validator: T;
}
interface WithValidatorOptional<T extends Validator> {
  validator?: T;
}

interface ExtendedElementTypeEntry
  extends WithChildren<Array<unknown>>,
    WithDataOptional<unknown>,
    WithIsInlineOptional<boolean>,
    WithIsVoidOptional<boolean> {}

interface ExtendedTextTypeEntry
  extends WithDataOptional<unknown>,
    WithText<string>,
    WithValidatorOptional<Validator> {}

type ExtendedComponentTypeEntry<T extends ExtendableComponentTypes> =
  T extends 'Text' ? ExtendedTextTypeEntry : ExtendedElementTypeEntry;

type ExtendedComponentTypeEntries<T extends ExtendableComponentTypes> = Record<
  string,
...
type ExtractDataProp<
  T extends ExtendedElementTypeEntry | ExtendedTextTypeEntry,
> = T extends WithData<infer U> ? U : EmptyProp;
...
type WithIsVoidProp<T extends ExtendedElementTypeEntry> = T extends WithIsVoid<
  infer U
>
  ? U extends true
    ? WithIsVoid<U>
...
type WithValidatorProp<T extends ExtendedTextTypeEntry> =
  T extends WithValidator<infer U> ? WithValidator<U> : EmptyProp;
...
export type ExtendableTypeIsExtended<T extends ExtendableTypes> =
  unknown extends CustomTypes[T] ? false : true;
...
> = ExtractDataProp<R[T]> & WithIsInlineProp<R[T]> & WithIsVoidProp<R[T]>;
...
export type ExtendedCellArgsType<
  T extends string,
  R extends Record<T, ExtendedElementTypeEntry>,
> = ExtractDataProp<R[T]> & WithIsInlineProp<R[T]> & WithIsVoidProp<R[T]>;
...
export type ExtendedTextArgsType<
  T extends string,
  R extends Record<T, ExtendedTextTypeEntry>,
> = ExtractDataProp<R[T]> & WithValidatorProp<R[T]>;

export type ExtendedTextType<
  T extends string,
  R extends Record<T, ExtendedTextTypeEntry>,
> = ExtendedTextArgsType<T, R> & WithText<R[T]['text']> & WithType<T>;
