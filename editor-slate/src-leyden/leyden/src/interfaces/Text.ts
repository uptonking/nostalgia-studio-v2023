...
export const Text: TextInterface = {
  ...
  validateTextOperation(
    ...
    const { offset, text: opText, type } = operation;
    if (opText.length === 0) {
      return true;
    }
    if (type === 'insert_text') {
      return validator(
        text.text.slice(0, offset) + opText + text.text.slice(offset),
      );
    }
    return validator(
      text.text.slice(0, offset) + text.text.slice(offset + opText.length),
    );
  },
};
