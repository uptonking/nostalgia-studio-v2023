...
export const TableTransforms: TableTransforms = {
  ...
  insertRows(
    ...
    if (hangingCells !== 0) {
      throw new Error(
        `failed to insert row, (${cells.length} cells cannot make rows of ${table.columns})`,
      );
    }
    ...
    if (options) {
      ...
      insertionCoords = {
        x: 0,
        y: Table.dimensions(LeydenEditor.table(editor)).rows,
      };
    }

    const insertionPath = LeydenEditor.cellPath(editor, {
      at: insertionCoords,
    });
  ...
  swapRows(
    ...
    Editor.withoutNormalizing(editor, () => {
      const rowsToSwap = a > b ? { top: b, bottom: a } : { top: a, bottom: b };
