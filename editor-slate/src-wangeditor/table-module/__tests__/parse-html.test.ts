...
describe('table - parse html', () => {
  ...
  it('table cell', () => {
    ...
    expect(
      parseCellHtmlConf.parseElemHtml($cell2[0], children, editor),
    ).toEqual({
      type: 'table-cell',
      isHeader: true,
      colSpan: 1,
  ...
  it('table row', () => {
    const $tr = $('<tr></tr>');
    const children = [
      { type: 'table-cell', children: [{ text: 'hello world' }] },
    ];
  ...
  it('table', () => {
    ...
    expect(
      parseTableHtmlConf.parseElemHtml($table[0], children, editor),
    ).toEqual({
      type: 'table',
      width: '100%',
      children,
