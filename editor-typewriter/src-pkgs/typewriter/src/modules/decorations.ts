...
const formatDecoration: FormatType = {
  ...
  render: (attributes, children) => {
    return applyDecorations(h('span', {}, children), attributes, [
      'format',
      'decoration',
    ]);
  },
...
export class Decorator {
  ...
  hasDecorations() {
    return (
      (Boolean(this._decoration) && this._decoration.ops.length > 0) ||
      this.change.delta.ops.length > 0
    );
  }

  getDecoration() {
    return this._decoration
      ? this._decoration.compose(this.change.delta)
      : this.change.delta;
  }
  ...
  clearLines(lines: Line[]) {
    ...
    const doc = this._doc;
    const range = [
      doc.getLineRange(lines[0])[0],
      doc.getLineRange(lines[lines.length - 1])[1],
    ] as EditorRange;
    const contiguous =
      lines.length === 1 ||
      lines.every(
  ...
  decorateText(
    range: EditorRange,
    decoration: Decorations = { class: this._name },
  ) {
  ...
  decorateLine(
    ...
    this.change.formatLine(
      range,
      { decoration: { [this._name]: decoration } },
      true,
    );
    return this;
  }
