...
export function initHistory(initOptions: Partial<Options> = {}) {
  return function (editor: Editor) {
    ...
    function record(change: TextChange, oldDoc: TextDocument) {
      ...
      const undo = new TextChange(
        null,
        change.delta.invert(oldDoc.toDelta()),
        oldDoc.selection,
      );
      ...
      if (
        lastRecorded &&
        (!options.delay || lastRecorded + options.delay > timestamp) &&
        stack.undo.length
      ) {
...
export function transformHistoryStack(
  stack: UndoStack,
