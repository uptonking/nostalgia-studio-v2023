...
export function input(editor: Editor) {
  ...
  function onMutate(list: MutationRecord[]) {
    ...
    if (change && change.ops.length) {
      ...
      editor.update(
        new TextChange(editor.doc, change, selection, editor.activeFormats),
        Source.input,
      );
...
function getTextChangeMutation(list: MutationRecord[]) {
  ...
  if (!text) return null;
  const textAdd = list.find(
    (record) =>
      record.addedNodes.length === 1 &&
      record.addedNodes[0].nodeName === '#text',
  );
  const brAddRemove = list.find((record) => {
    return (
      (record.addedNodes.length === 1 &&
        record.addedNodes[0].nodeName === 'BR') ||
      (record.removedNodes.length === 1 &&
        record.removedNodes[0].nodeName === 'BR')
    );
  });
...
function getChangedLineRange(
...
): HTMLLineRange | undefined {
  let start: HTMLLineElement | undefined;
  let end: HTMLLineElement | undefined;
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (line && line.key) {
      if (
        !start ||
        getLineNodeStart(root, line) < getLineNodeStart(root, start)
      )
        start = line;
