...
const EMPTY_OBJ = {};
const IS_CHROME =
  (window as any).chrome && typeof (window as any).chrome === 'object';
...
export function keyboard(editor: Editor) {
  function onEnter(event: KeyboardEvent) {
    ...
    if (at === start && to === end && type.frozen) {
      ...
      attributes = type.nextLineAttributes
        ? type.nextLineAttributes(attributes)
        : EMPTY_OBJ;
    } else if (
      atEnd &&
      (type.nextLineAttributes || type.defaultFollows || type.frozen)
    ) {
      attributes = type.nextLineAttributes
        ? type.nextLineAttributes(attributes)
        : EMPTY_OBJ;
  ...
  function handleDelete(event: KeyboardEvent, direction: 1 | -1) {
    ...
    if (
      isCollapsed &&
      (!IS_CHROME || event.ctrlKey || event.altKey || event.metaKey)
    ) {
  ...
  function isEmpty(line: Line) {
    return (
      line.length === 1 &&
      !editor.typeset.lines.findByAttributes(line.attributes)?.frozen
    );
  }
