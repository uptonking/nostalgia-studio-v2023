...
export function paste(editor: Editor, options?: PasteOptions) {
  function paste({ selection, text, html }: PasteOptions) {
    ...
    if (hasLines) {
      ...
      let lines = lineUtils.fromDelta(delta, doc.byId);
      delta = lineUtils.toDelta(lines);
      length = delta.length();
      ...
      const endAttrs =
        startLine === endLine ? startAttrs : getAttributes(endLine);
      ...
      const pastedEndAttrs =
        pastedStartLine === pastedEndLine
          ? pastedStartAttrs
          : getAttributes(pastedEndLine);

      if (
        at !== doc.getLineRange(startLine)[0] &&
        !isEqual(startAttrs, pastedStartAttrs, ignoreId)
      ) {
      ...
      if (
        endsInNewline &&
        to !== doc.getLineRange(endLine)[1] &&
        isEqual(endAttrs, pastedEndAttrs, ignoreId)
      ) {
    ...
    const viewEvent = new PasteEvent('paste', {
      delta,
      html,
      text,
      cancelable: true,
    ...
    if (!viewEvent.defaultPrevented) {
      if (delta && delta.ops.length) {
        const change = editor.change.delete(
          selection,
          hasLines ? dontFixNewline : undefined,
        );
  ...
  function onPaste(event: ClipboardEvent) {
    if (!editor.enabled || !editor.doc.selection || event.defaultPrevented)
      return;
