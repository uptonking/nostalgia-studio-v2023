...
export function selection(editor: Editor) {
  ...
  function onDecorate() {
    ...
    const decorator = (
      editor.modules.decorations as DecorationsModule
    ).getDecorator('selection');
    decorator.clear();
    ...
    if (selection) {
      doc.getLinesAt(selection).forEach((line) => {
        if (lines.findByAttributes(line.attributes, true).frozen) {
          ...
          decorator.decorateLine(doc.getLineRange(line)[0], {
            class: 'selected' + (focused ? ' focus' : ''),
          });
        }
      });
    }
  ...
  function pause() {
    ...
    const { decorations } = editor.modules as {
