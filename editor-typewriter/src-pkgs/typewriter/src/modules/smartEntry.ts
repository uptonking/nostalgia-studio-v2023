...
const httpExpr =
  /(https?:\/\/.)[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b[-a-zA-Z0-9@:%_+.~#?&/=]*\s$/s;
const wwwExpr =
  /(www\.)[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b[-a-zA-Z0-9@:%_+.~#?&/=]*\s$/s;
const nakedExpr =
  /[-a-zA-Z0-9@:%._\+~#=]{2,256}\.(com|org|net|io)\b[-a-zA-Z0-9@:%_+.~#?&/=]*\s$/s;
...
export const lineReplacements: Replacement[] = [
  ...
  [
    ...
    (char, { indent }) => ({
      ...
      indent,
      start:
        char === 'A' ? undefined : char.charCodeAt(0) - 'A'.charCodeAt(0) + 1,
    }),
  ],
  [
    /^([a-z])\. $/,
  ...
  return lineReplacements.some(([regexp, getAttributes]) => {
    ...
    if (match) {
      const attributes = getAttributes(
        match[1],
        editor.doc.getLineFormat(index),
      );
...
export function markReplace(
  ...
  return markReplacements.some(([regexp, getAttributes]) => {
    ...
    if (match) {
      ...
      const attributes = getAttributes(
        matched,
        editor.doc.getTextFormat(index),
      );
      if (!editor.typeset.formats.findByAttributes(attributes)) {
        return false;
      }
      const selection = index - (text.length - matched.length) + last.length;
      if (last === ' ' && wholeText[index] === ' ') last = '';
      const end = index - last.length;
      editor.insert(matched, attributes, [
...
export function textReplace(editor: Editor, index: number, prefix: string) {
  return textReplacements.some(([regexp, replaceWith]) => {
    const match = prefix.match(regexp);
    if (match) {
      editor.insert(replaceWith(match[1]), undefined, [
