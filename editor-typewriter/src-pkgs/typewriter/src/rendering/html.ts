...
function isLastNode(editor: Editor, node: Node) {
  const containingLine =
    ...
    (node as Element).closest(editor.typeset.lines.selector);
...
export function deltaFromHTML(
  editor: Editor,
  html: string,
  options?: DeltaFromHTMLOptions,
) {
...
export function fromNode(editor: Editor, dom: HTMLElement) {
  const lines = lineUtils.fromDelta(
    deltaFromDom(editor, { root: dom }),
    editor.doc.byId,
  );
...
export function deltaFromDom(
  editor: Editor,
  options: FromDomOptions = defaultOptions,
): Delta {
  ...
  const walker = createTreeWalker(
    root,
    (node) => !SKIP_ELEMENTS[node.nodeName],
  );
  ...
  while ((node = walker.nextNode())) {
    ...
    if (isBRPlaceholder(editor, node)) {
      ...
      node.nodeName === 'BR' &&
      (node as Element).className === 'Apple-interchange-newline'
    ) {
      delta.insert(
        '\n',
      ...
      const filteredWhitespace = collapseWhitespace
        ? nodeText.replace(whitespaceExp, ' ')
        : nodeText;
      ...
      if (embed) {
        const attributes = gatherFormats(
        ...
        if (embed.fromDom !== false) {
          delta.insert(
            embed.fromDom ? embed.fromDom(node) : { [embed.name]: true },
            attributes,
          );
        }
      }
      ...
      (node.nodeType === Node.ELEMENT_NODE &&
        (node as Element).matches(BLOCK_ELEMENTS))
    ) {
      ...
      if (
        line === lines.default &&
        (!node.parentNode || lines.matches(node.parentNode))
      ) {
        continue;
      }
...
function childrenToHTML(children: VChild[]): string {
  ...
  return (children as any).reduce(
    (html: string, child: VChild) =>
      html +
      (typeof child !== 'string'
        ? nodeToHTML(child)
        : escapeHtml(child).replace(/\xA0/g, '&nbsp;')),
    '',
  );
}
...
function nodeToHTML(node: VChild): string {
  ...
  const attr = Object.keys(node.props).reduce(
    (attr, name) =>
  ...
  const closingTag =
    children || !VOID_ELEMENTS[node.type] ? `</${node.type}>` : '';
  ...
  while (parent && !lines.matches(parent) && parent !== root) {
    if (formats.matches(parent)) {
      ...
      if (format && format.fromDom !== false) {
        attributes[format.name] = format.fromDom
          ? format.fromDom(parent)
          : true;
      }
