...
export function getIndexFromPoint(editor: Editor, x: number, y: number) {
  ...
  if (document.caretRangeFromPoint) {
    ...
    if (range) {
      return getIndexFromNodeAndOffset(
        editor,
...
export function getLineInfoFromPoint(
  editor: Editor,
  y: number,
): LineInfo | undefined {
  const { root } = editor;
  ...
  const lineElements = Array.from(
    root.querySelectorAll(editor.typeset.lines.selector),
...
export function getBrowserRange(editor: Editor, range: EditorRange) {
  ...
  const [anchorNode, anchorOffset, focusNode, focusOffset] = getNodesForRange(
    editor,
    range,
  );
...
export function getBoundingBrowserRange(
  editor: Editor,
  range: EditorRange,
): Range {
...
export function getIndexFromNode(editor: Editor, startNode: Node): number {
  ...
  while ((node = walker.previousNode())) {
    if (node === root) break;
      ...
      embeds.matches(node) &&
      !isBRPlaceholder(editor, node as HTMLElement)
    )
      index++;
    else if (
      lines.matches(node) &&
      editor.doc.lines[0].id !== (node as any).key
    )
      index++;
  }
  return index;
...
export function getLineElementAt(editor: Editor, index: number) {
  ...
  return childNodes.find(
    (line: HTMLLineElement) =>
      getLineNodeStart(root, line) <= index &&
      getLineNodeEnd(root, line) > index,
  ) as HTMLLineElement;
}
...
export function getNodeLength(editor: Editor, parentNode: Node): number {
  const { lines, embeds } = editor.typeset;
  if (
    embeds.matches(parentNode) &&
    !isBRPlaceholder(editor, parentNode as HTMLElement)
  ) {
    return 1;
  }
...
export function getNodesForRange(
  editor: Editor,
  range: EditorRange,
  ...
  if (range == null) {
    ...
    const [anchorNode, anchorOffset, frozen] = getNodeAndOffset(
      editor,
      range[0],
      anchorFirst ? 0 : 1,
    );
...
export function textNodeLength(lines: Types, node: Node) {
  ...
  if (
    value.trim() ||
    !(lines.matches(node.previousSibling) || lines.matches(node.nextSibling))
  ) {
    return value.length;
  }
  return 0;
