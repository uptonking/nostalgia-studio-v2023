...
export function setLineNodesRanges(editor: Editor) {
  ...
  for (let i = 0; i < root.children.length; i++) {
    ...
    if (Array.isArray(entry)) {
      ...
      ranges.set(child, [
        doc.getLineRange(entry[0])[0],
        doc.getLineRange(entry[entry.length - 1])[1],
      ]);

      // set the ranges for each line inside
      const lineElements = child.querySelectorAll(
        editor.typeset.lines.selector,
      ) as any as HTMLLineElement[];
  ...
  const lineElements = root.querySelectorAll(
    editor.typeset.lines.selector,
  ) as any as HTMLLineElement[];
  for (let i = 0; i < lineElements.length; i++) {
    const lineElement = lineElements[i];
...
export function renderDoc(
  ...
  return renderCombined(
    editor,
    combineLines(editor, doc.lines).combined,
    forHTML,
  );
}
...
export function renderCombined(
  ...
  return combined
    .map((line) => renderLine(editor, line, forHTML))
...
export function renderLine(
  ...
  return Array.isArray(line)
    ? renderMultiLine(editor, line, forHTML)
...
export function combineLines(editor: Editor, lines: Line[]): CombinedData {
  ...
  lines.forEach((line, i) => {
    ...
    if (type.shouldCombine) {
      ...
      if (
        !next ||
        getLineType(editor, next) !== type ||
        !type.shouldCombine(collect[0].attributes, next.attributes)
      ) {
        // By keeping the last array reference we can optimize updates
        const last = linesMultiples.get(collect[0]);
        if (
          last &&
          last.length === collect.length &&
          collect.every((v, i) => last[i] === v)
        ) {
          collect = last;
        } else {
