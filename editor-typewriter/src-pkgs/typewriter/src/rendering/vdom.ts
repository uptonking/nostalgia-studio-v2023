...
const getKey = (vdom: VChild | Node) =>
  vdom == null ? vdom : (vdom as any).key;
...
const patchProp = (
  ...
  if (key === 'key') {
    ...
    if (
      !(((dom as any).events || ((dom as any).events = {}))[
        (key = key.slice(2))
      ] = newVal)
    ) {
...
const createNode = (vdom: VChild, isSvg?: boolean) => {
  ...
  const props = (vdom as VNode).props;
  const dom = (isSvg = isSvg || vdom.type === 'svg')
  ...
  vdom.children.forEach((kid) =>
    dom.appendChild(createNode(vdomify(kid), isSvg)),
  );
...
const patchDom = (
  ...
  if (typeof newVdom === 'string') {
    ...
    oldDom == null ||
    oldDom.nodeName.toLowerCase() !== (newVdom as VNode).type
  ) {
    ...
    const oldProps = getDomProps(oldDom as Element, isSvg);
    const newProps = newVdom.props;

    isSvg = isSvg || newVdom.type === 'svg';

    for (const i in { ...oldProps, ...newProps }) {
      if ((domProps.has(i) ? dom[i] : oldProps[i]) !== newProps[i]) {
...
const patchChildren = (
  ...
  if (oldHead > oldTail) {
    ...
    const insertBefore =
      oldKids[oldHead] ||
      (oldKids[oldHead - 1] && oldKids[oldHead - 1].nextSibling) ||
      null;
...
export const patch = (
  ...
  if (Array.isArray(vdom)) {
    dom = patchChildren(
      dom,
      vdom,
      dom instanceof window.SVGElement,
      oldKids as Node[],
    );
  } else {
