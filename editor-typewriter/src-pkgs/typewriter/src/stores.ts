...
export function focusStore(selection: Readable<EditorRange | null>) {
  return derived(selection, (selection) => Boolean(selection));
}
...
export function proxy<T>(defaultValueOrStore: T | Readable<T>) {
  const isReadable =
    typeof (defaultValueOrStore as Readable<T>).subscribe === 'function';
  const defaultValue = isReadable
    ? get(defaultValueOrStore as Readable<T>)
    : (defaultValueOrStore as T);
