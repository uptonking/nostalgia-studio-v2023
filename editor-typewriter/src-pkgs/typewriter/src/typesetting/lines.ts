...
export const list = registerLineType({
  ...
  fromDom(node: HTMLElement) {
    ...
    const type = parent && (parent as Element).getAttribute('type');
  ...
  shouldCombine: (prev, next) =>
    (prev.list === next.list && !next.start && prev.type === next.type) ||
    next.indent,
    ...
    lists.forEach(([attributes, children, id]) => {
      ...
      if (attributes.list === 'check') {
        ...
        const check = h('button', {
          class: 'check-list-check',
          onmousedown: toggle,
          ontouchstart: toggle,
        });
        if (children.length === 1 && (children[0] as VNode).type === 'br')
          children.push(check);
        else children.unshift(check);
        props = {
          ...props,
          class: 'check-list-item',
          ['data-checked']: String(attributes.checked || false),
        };
      }
      const item = applyDecorations(h('li', props, children), attributes);

      while (index >= levels.length) {
        const newLevel = h(type, {
          start: attributes.start,
          type: attributes.type,
          key: `${id}-outer`,
        });
        const childrenArray = levels.length
...
export const blockquote = registerLineType({
  ...
  renderMultiple: (quotes) => {
    ...
    const props =
      typeof type === 'string' ? { className: `quote-${type}` } : null;
    const children = quotes.map(([attributes, children, id]) =>
      h('p', { key: id }, children),
    );
  ...
  renderMultiple: (lines) => {
    ...
    lines.forEach(([attributes, inlineChildren, id]) => {
      if (
        ...
        (inlineChildren[inlineChildren.length - 1] as VNode).type === 'br'
      ) {
        inlineChildren.pop();
      }
