...
/**
 * Typeset holds the rules for what types of content is allowed in the editor
 * and how that content is mapped to HTML and back again.
 */
export class Typeset {
  /** formats which apply to whole lines. Things like headers, blockquotes, paragraphs, and list items. */
  lines: Types<LineType>;
  /** inline formatting such as bold, italic, and link */
  formats: Types<FormatType>;
  /**  inline content that cannot be represented with simple text, such as images and line breaks (<br>)
  ...
  constructor(types: TypesetTypes) {
    const lines = types.lines
      ?.map((entry) => (typeof entry === 'string' ? lineTypes[entry] : entry))
      .filter(Boolean);
    const formats = types.formats
      ?.map((entry) => (typeof entry === 'string' ? formatTypes[entry] : entry))
      .filter(Boolean);
    const embeds = types.embeds
      ?.map((entry) => (typeof entry === 'string' ? embedTypes[entry] : entry))
      .filter(Boolean);
...
export class Types<T extends BasicType = BasicType> {
  ...
  init() {
    ...
    this.priorities = this.list.reduce((priorities, type, i) => {
