...
/** listeners Map shared by all EventDispatcher instances */
...
export class EventDispatcher {
  ...
  removeEventListener(
    ...
    if (options?.once)
      listener = getOnceListener(this, type, listener) as EventListener;
    if (!listener) return;
...
function getOnceListener(
  ...
  if (!events && autocreate)
    dispatcherEvents.set(obj, (events = Object.create(null)));
  const map =
    (events && events[type]) || (autocreate && (events[type] = new Map()));
