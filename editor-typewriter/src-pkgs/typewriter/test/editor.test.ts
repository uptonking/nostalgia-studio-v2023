...
import { Editor, type EditorChangeEvent } from '../src/editor';
...
describe('======== Editor ========', () => {
  ...
  describe('select', () => {
    ...
    it('should dispatch "change" when there is a selection change', () => {
      ...
      editor.on(
        ...
        (event: EditorChangeEvent) =>
          (dispatched = event.change!.selectionChanged),
      );
      editor.select([1, 5]);
    ...
    it('should update activeFormats when the selection changes', () => {
      editor.insert('This is a test!');
      editor.update(
        editor.change.formatText([10, 14], { bold: true, italic: true }),
      );
  ...
  describe('setDelta', () => {
    it('should set the contents', () => {
      const delta = new Delta()
        .insert('This is a ')
        .insert('test', { bold: true, italic: true })
        .insert('!');
  ...
  describe('insert text', () => {
    it('should insert text into a blank document', () => {
      ...
      expect(editor.getDelta().ops).to.deep.equal([{ insert: 'Testing\n' }]);
    });
  ...
  describe('delete', () => {
    it('should delete text', () => {
      ...
      expect(editor.getDelta().ops).to.deep.equal([{ insert: 'Test out\n' }]);
    });
