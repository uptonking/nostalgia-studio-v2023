...
export async function stripeCreatePaymentIntent(
  ...
  try {
    ...
    const err = e as Stripe.errors.StripeAPIError & {
      raw?: { message: string; type: string };
    };
...
export async function cleanProducts(
  ...
  const toRemove = existing.rows
    .filter(
      (p) =>
        !productsCache.find((c) => c.productId === p.getDataValue('productId')),
    )
    .map((p) => p.getDataValue('productId'));

  if (toRemove.length > 0) {
    try {
      const resu = (await Connection.db
        .getQueryInterface()
        .bulkDelete('products', {
          product_id: toRemove,
        })) as { rowCount: number }[];
