...
const get = <T>(url: string) =>
  axios.get<T>(`${config.auth?.baseUrl}/api/v2/${url}`, readOptions());
...
export async function authProviderAutoConfigure(): Promise<boolean> {
  ...
  if (
    !config.auth.tenant ||
    !config.auth.explorerId ||
    !config.auth.explorerSecret
  ) {
...
async function ensureClients() {
  ...
  const grant = async (client: AuthClient, audience: string) => {
    ...
    if (grantResult.data?.id) {
      ...
      logger.error(
        `Failed to create client grant for ${client.name}` +
          JSON.stringify(grantResult.data),
      );
    }
  };
  ...
  const addClient = async (client: Partial<AuthClient>, audience: string) => {
    ...
    if (!result.data?.client_id) {
      logger.error(
        `Failed to create client ${client.name}` + JSON.stringify(result.data),
      );
    }
  ...
  const authManager = existing.data.find(
    (c) => c.name === 'API Explorer Application',
  );
  let existingBackend = existing.data.find((e) => e.name === 'backend');
  if (!existingBackend) {
    existingBackend = await addClient(
      backendClient,
      config.auth.explorerAudience,
    );
  }
  ...
  if (
    ...
    !grants.find(
      (g) =>
        g.client_id === existingBackend?.client_id &&
        g.audience === authManager.identifier,
    )
  ) {
  ...
  if (
    ...
    !grants.find(
      (g) =>
        g.client_id === existingClient?.client_id &&
        g.audience === config.auth.clientAudience,
    )
  ) {
...
async function ensureResourceServers() {
  ...
  if (missing.length) {
    log(
      `Creating missing resource servers: ${missing
        .map((m) => m.name)
        .join(', ')}`,
    );
...
async function ensureRules() {
  ...
  const missing = rules.filter(
    (rule) => !existing.data.find((erule) => erule.name === rule.name),
  );
  if (missing.length) {
