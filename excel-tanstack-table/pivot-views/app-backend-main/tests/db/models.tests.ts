...
import {
  ...
  deleteIfExists,
  getIfExists,
} from 'src/shared/model-api/controller';
...
export function getPopulatedModel(
  model: ModelStatic<Model>,
  cachedKeys: Record<string, unknown>,
) {
...
describe('Entity CRUD', () => {
  ...
  for (const entity of sorted) {
    ...
    test(`update ${model.name}`, async () => {
      ...
      const propName = Object.keys(attributes).find(
        (key) =>
          key !== model.primaryKeyAttribute && !foreignKeys.includes(key),
      );
      if (!propName) {
  ...
  for (const entity of reversed) {
    ...
    test(`delete ${model.name}`, async () => {
      const result = await deleteIfExists(
        model,
        mock[model.primaryKeyAttribute] as string,
      );
