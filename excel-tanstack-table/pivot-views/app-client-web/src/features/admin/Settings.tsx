...
import { debounceFn } from '../../shared/debounce';
...
export function Settings() {
  ...
  const [data, setData] =
    React.useState<{ [k in SettingType]: SettingData[k] }>();
  ...
  const save = (name: SettingType, prop: string, value: unknown) => {
    ...
    const newData = { ...data, [name]: newValue } as {
      [k in SettingType]: SettingData[k];
    };
    const setting = { name, data: newValue } as Setting;
    setData(newData);
    debounceFn(setting.name, () => saveAsync(setting));
  };
            ...
            <CardContent>
              <Grid container>
                <Grid item xs={4} md={5}>
                  <FormGroup>
                    <FormControlLabel
                      control={
                        <Switch
                          checked={Boolean(data?.system?.disable)}
                          onChange={() =>
                            save('system', 'disable', !data?.system?.disable)
                          }
                        />
                      }
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          checked={Boolean(data?.system?.enableCookieConsent)}
                          onChange={() =>
                            save(
                              'system',
                              'enableCookieConsent',
                ...
                <Grid item xs={4} md={5}>
                  <FormGroup>
                    <FormControlLabel
                      control={
                        <Switch
                          checked={Boolean(data?.system?.enableStore)}
                          onChange={() =>
                            save(
                              'system',
                              'enableStore',
                              !data?.system?.enableStore,
                            )
                          }
                        />
                      }
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          checked={Boolean(data?.system?.enableShippingAddress)}
                          onChange={() =>
                            save(
                              'system',
                ...
                <Grid
                  ...
                  md={2}
                  sx={{
                    backgroundColor: data?.system?.disable
                      ? 'error.dark'
                      : 'success.dark',
                    borderRadius: '10px',
                    display: 'flex',
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs={4} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(data?.system?.enableRegistration)}
                        onChange={() =>
                          save(
                            'system',
                            'enableRegistration',
                ...
                <Grid item xs={4} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(data?.auth0?.enabled)}
                        onChange={() =>
                          save('auth0', 'enabled', !data?.auth0?.enabled)
                        }
                      />
                    }
                    label='Enable'
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('auth0', 'clientAudience', e.target.value)
                    }
                  />
                </Grid>
                <Grid item xs={8}>
                ...
                <Grid item xs={4} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(data?.auth0?.sync)}
                        onChange={() =>
                          save('auth0', 'sync', !data?.auth0?.sync)
                        }
                      />
                    }
                    label='Enable'
                ...
                <Grid item xs={12}>
                  <Typography variant='body2' component='p' mb={1}>
                    Auto configure application and create resources in Auth0.
                    Only needs the `Client ID` and `Client Secret` from the API
                    ...
                    and go to API Explorer Application - Settings - copy Client
                    ID and Secret
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('auth0', 'explorerId', e.target.value)
                    }
                  />
                </Grid>
                <Grid item xs={12}>
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs={3} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(data?.google?.enabled)}
                        onChange={() =>
                          save('google', 'enabled', !data?.google?.enabled)
                        }
                      />
                    }
                    label='Enable'
                ...
                <Grid item xs={3} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(data?.system?.enableOneTapLogin)}
                        onChange={() =>
                          save(
                            'system',
                            'enableOneTapLogin',
                            !data?.system?.enableOneTapLogin,
                          )
                        }
                      />
                    }
                ...
                <Grid item xs={12}>
                  <Typography variant='body2' component='p'>
                    <Link
                      ...
                      target='_blank'
                    >
                      Project OAuth 2.0 Client ID and Secret (Click Create
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('google', 'projectId', e.target.value)
                    }
                  />
                </Grid>
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('google', 'analyticsId', e.target.value)
                    }
                  />
                </Grid>
              </Grid>
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs={3} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(
                          data?.system?.paymentMethods?.stripe
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    fullWidth
                    value={
                      data?.system?.paymentMethods?.stripe?.publishableKey || ''
                    }
                    onChange={(e) =>
                      save(
                        'system',
                        'paymentMethods.stripe.publishableKey',
                        e.target.value,
                      )
                    }
                  />
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('internal', 'secrets.stripe.apiKey', e.target.value)
                    }
                  />
                </Grid>
              </Grid>
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs={3} sx={{ textAlign: 'right' }}>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={Boolean(
                          data?.system?.paymentMethods?.paypal
                            ?.subscriptionsEnabled,
                        )}
                        onChange={() =>
                          save(
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      save('internal', 'secrets.paypal.apiKey', e.target.value)
                    }
                  />
                </Grid>
              </Grid>
