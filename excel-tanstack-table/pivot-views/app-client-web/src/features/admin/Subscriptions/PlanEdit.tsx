    ...
    if (typeof setState === 'function') {
      setState({
        ...
        mappings: {
          ...item?.mappings,
          [source]: {
            ...item?.mappings[source as PaymentSource],
            [field]: value,
          },
        },
      });
            ...
            {Object.keys(PaymentSources).map((source) => (
              <Grid container key={source}>
                ...
                <Grid item xs={2}>
                  <TextField
                    value={
                      item?.mappings[source as PaymentSource]?.productId || ''
                    }
                    onChange={(e) =>
                      handleMapChange(source, 'productId', e.target.value)
                    }
                  />
                </Grid>
                <Grid item xs={2}>
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={(e) =>
                          handleMapChange(source, 'enabled', e.target.value)
                        }
                      />
                    }
                    label='Enabled'
