...
export function Details() {
  ...
  const update = async (id?: string, field?: string, value?: unknown) => {
    const response = await request<Drawing>(
      '/drawing',
      { id, field, value },
      Method.PATCH,
    );
      ...
      >
        ...
        <FormGroup>
          <TextField
            ...
            value={item?.price || 0}
            onChange={(e) =>
              update(item.drawingId, 'price', Number(e.target.value))
            }
            InputProps={{
              startAdornment: (
          ...
          <FormControlLabel
            control={
              <Switch
                onChange={(e) =>
                  update(item.drawingId, 'sell', e.target.checked)
                }
                checked={!!item?.sell}
          ...
          <FormControlLabel
            control={
              <Switch
                name='private'
                onChange={(e) =>
                  update(item.drawingId, 'private', e.target.checked)
                }
