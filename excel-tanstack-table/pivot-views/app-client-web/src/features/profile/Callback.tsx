...
export default function Callback(): JSX.Element {
  const clientId = useAppSelector(
    (state) => state.app.settings?.auth0?.clientId,
  );
  ...
  const state = new URLSearchParams(window.location.href).get(
    'state',
  ) as string;
  const error = new URLSearchParams(window.location.hash).get('#error');
  const errorDescription = new URLSearchParams(window.location.href).get(
    'error_description',
  );
  React.useEffect(() => {
