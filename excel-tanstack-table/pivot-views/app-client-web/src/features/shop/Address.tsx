...
export default function AddressForm() {
  ...
  const shippingAddressId = useAppSelector(
    (store) => store.shop.shippingAddressId,
  );
  const shippingAddress =
    items?.find((a) => a.addressId === shippingAddressId) || newAddress();
  ...
  const save = (item: Address) => {
    ...
    const addresses =
      items?.map(
        (a) => (a.addressId === item.addressId ? item : a) as Address,
      ) || [];
    ...
    debounceFn('address', () => request('address', item), 1000);
  };
  ...
  const deleteHandler = (item: Address) => {
    const addresses =
      items?.filter((a) => a.addressId !== item.addressId) || [];
    dispatch(patch({ addresses }));
    if (selected.addressId === item.addressId) {
      setSelected(
        addresses.find((a) => a.addressId !== item.addressId) || newAddress(),
      );
    }
