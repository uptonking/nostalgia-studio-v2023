...
export const loadAsync = createAsyncThunk(
  ...
  async (_, { dispatch, getState }) => {
    ...
    const { data: cart } = await get<PagedResult<Cart>>(
      'cart?include=drawing,product',
    );
...
export const cartAsync = createAsyncThunk(
  'shop/cart',
  async (
    { drawing, product, quantity, ...item }: Partial<Cart>,
    { dispatch, getState },
  ) => {
    const state = getState() as RootState;
    ...
    const existing = state.shop.items.find(
      (i) =>
        ...
        i.priceId === product?.id ||
        i.drawingId === drawing?.drawingId,
    );
    if (existing != null) {
...
export const checkoutAsync = createAsyncThunk(
  ...
  async (payload: PaymentIntentResult, { dispatch, getState }) => {
    ...
    const response = await request<Order, CheckoutRequest>(
      'shop/checkout',
      checkout,
    );
