    ...
    >
      <Container maxWidth='xl'>
        <Toolbar disableGutters>
          ...
          {/* 👇🏻 use duplicate component with different styles to implement responsive */}
          ...
          <Box sx={{ flexGrow: 0, ml: 1 }}>
            ...
            >
              {profileLinks
                .filter((r) =>
                  r.secure ? authenticated : authenticated ? !r.anon : true,
                )
                .filter((route) =>
                  route.roles ? route.roles.every((r) => hasRole(r)) : true,
                )
                .filter((r) =>
                  !enableAuth || !enableRegistrations
                    ? !['/register'].includes(r.path)
                    : true,
                )
                .map((setting) => {
                  // console.log(';; setting')
