...
} from '../../field/filter/attachment.filter';
...
export class HasFileType extends BaseRecordQuerySpecification<IAttachmentFilterTypeValue> {
  isSatisfiedBy(r: Record): boolean {
    ...
    return (
      value instanceof AttachmentFieldValue && value.hasFileType(this.value)
    );
  }
...
export class HasExtension extends BaseRecordQuerySpecification<string> {
  isSatisfiedBy(r: Record): boolean {
    ...
    return (
      value instanceof AttachmentFieldValue && value.hasExtension(this.value)
    );
  }
