...
test.each<[DateEqual, DateEqual, boolean]>([
  ...
  [
    new DateEqual('name', dateValue),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    false,
  ],
  [
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
...
test.each<[DateGreaterThan, DateEqual, boolean]>([
  [
    new DateGreaterThan('name', new DateFieldValue(addDays(date, 1))),
    new DateEqual('name', dateValue),
    false,
  ],
  [
    new DateGreaterThan('name', dateValue),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    true,
  ],
...
test.each<[DateGreaterThanOrEqual, DateEqual, boolean]>([
  [
    new DateGreaterThanOrEqual('name', new DateFieldValue(addDays(date, 1))),
    new DateEqual('name', dateValue),
    false,
  ],
  [
    new DateGreaterThanOrEqual('name', dateValue),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    true,
  ],
  [
...
test.each<[DateLessThan, DateEqual, boolean]>([
  [
    new DateLessThan('name', new DateFieldValue(addDays(date, 1))),
    new DateEqual('name', dateValue),
    true,
  ],
  [
    new DateLessThan('name', dateValue),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    false,
  ],
...
test.each<[DateLessThanOrEqual, DateEqual, boolean]>([
  [
    new DateLessThanOrEqual('name', new DateFieldValue(addDays(date, 1))),
    new DateEqual('name', dateValue),
    true,
  ],
  [
    new DateLessThanOrEqual('name', dateValue),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    false,
  ],
  [
...
test.each<[DateIsToday, DateEqual, boolean]>([
  ...
  [
    new DateIsToday('name'),
    new DateEqual('name', new DateFieldValue(addDays(date, 1))),
    false,
  ],
  [
    new DateIsToday('name'),
    new DateEqual('name', new DateFieldValue(addHours(date, 1))),
    true,
  ],
  [
    new DateIsToday('name'),
    new DateEqual('name', new DateFieldValue(startOfDay(date))),
    true,
  ],
  [
    new DateIsToday('name'),
    new DateEqual('name', new DateFieldValue(endOfDay(date))),
    true,
  ],
])('DateLessThanOrEqual.isSatisfiedBy', (spec, value, result) => {
