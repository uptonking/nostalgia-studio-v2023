...
export class ViewFieldOptions extends ValueObject<
  Map<string, IViewFieldOption>
> {
  ...
  public removeField(field: Field): Option<ViewFieldOptions> {
    if (this.value.has(field.id.value)) {
      const options = new ViewFieldOptions(
        new Map(
          [...this.value.entries()].filter(([k]) => k !== field.id.value),
        ),
      );
