...
export const FieldMenuDropdown: React.FC<IProps> = ({
  field,
  orientation,
  index,
  pinned,
      ...
      <Menu.Dropdown>
        ...
        {orientation === 'vertial' ? (
          ...
          <>
            <Menu.Item
              className={classes.menu}
              onClick={insertAt(index - 1)}
              icon={<IconColumnInsertLeft size={14} />}
            >
              {t('Insert Field Left')}
            </Menu.Item>
            <Menu.Item
        ...
        {pinLeft && (
          <Menu.Item
            ...
            onClick={pinLeft}
            icon={
              pinned ? <IconPinnedOff size={14} /> : <IconPinned size={14} />
            }
            sx={(theme) => ({
        ...
        {field.sortable && (
          <>
            <Menu.Item
              ...
              sx={(theme) => ({
                backgroundColor:
                  direction === 'asc' ? theme.colors.gray[0] : 'inherit',
              })}
              onClick={() => {
                if (direction === 'asc') {
                  resetFieldSort({
            ...
            <Menu.Item
              ...
              sx={(theme) => ({
                backgroundColor:
                  direction === 'desc' ? theme.colors.gray[0] : 'inherit',
              })}
              onClick={() => {
                if (direction === 'desc') {
                  resetFieldSort({
