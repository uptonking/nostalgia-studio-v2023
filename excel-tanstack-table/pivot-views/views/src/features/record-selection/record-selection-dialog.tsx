...
import { confirmModal, useAppDispatch, useAppSelector } from '../../hooks';
import { useCurrentTable } from '../../hooks/use-current-table';

export const RecordSelectionDialog: React.FC = () => {
  const table = useCurrentTable();
  const ids = useAppSelector((state) =>
    getTableSelectedRecordIdList(state, table.id.value),
  );
  const hasSelectedRecords = useAppSelector((state) =>
    getTableHasSelectedRecordIds(state, table.id.value),
  );
  const count = useAppSelector((state) =>
    getTableSelectedRecordIdsCount(state, table.id.value),
  );
      ...
      <Group position='apart'>
        <Group spacing='xs'>
          <Text size='sm' fw={500} color='gray.7'>
            {t('Selected N Records', {
              n: count === 0 ? previousCount : count,
            })}
          </Text>
          <Button
            compact
            ...
            color='dark'
            variant='outline'
            onClick={() => dispatch(resetSelectedRecordIds())}
          >
            {t('Reset', { ns: 'common' })}
          </Button>
        </Group>
