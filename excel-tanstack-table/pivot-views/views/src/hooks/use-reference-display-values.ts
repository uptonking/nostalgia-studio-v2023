...
export const useReferenceDisplayValues = (
  ...
  const { data: record } = useGetRecordQuery(
    { tableId: table.id.value, id: recordId },
    { skip: !recordId },
  );
  ...
  if (record) {
    ...
    if (Array.isArray(foreignRecordIds) && !isEmpty(foreignRecordIds)) {
      ...
      for (const [index, foreignRecordId] of foreignRecordIds.entries()) {
        data.push({
          value: foreignRecordId as string,
          label: values[index]?.filter(Boolean).toString() || 'Unnamed',
        });
      }
    } else if (isString(foreignRecordIds)) {
      const values = field.getDisplayValues(record.displayValues);
      data.push({
        value: foreignRecordIds,
        label: values[0]?.filter(Boolean).toString() || 'Unnamed',
      });
    }
  }
      ...
      [
        {
          ...
          label: values.filter(Boolean).toString() || 'Unnamed',
        },
      ],
      'value',
    );
