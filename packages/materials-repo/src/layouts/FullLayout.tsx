...
const FullLayout = (props) => {
  ...
  // console.log(';;pps4 FullLayout, ', auth);

  const [width, setWidth] = useState(window.innerWidth);

  useEffect(() => {
    // if (pathname === '/dashboard/basic' && auth.isAuthenticated) {
    if (pathname === '/dashboard/basic') {
  ...
  // 必须放在下面的redirect之前，否则eslint会提示hooks order不确定
