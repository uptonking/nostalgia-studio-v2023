...
export function Login() {
  ...
  const handleSubmitLoginForm = useCallback(
    (event, userInfo?: UserType) => {
      if (event) {
        event.preventDefault();
      }
      // 防止重复提交
      if (isLoading) return;
      if ((!username.trim() || !password.trim()) && !userInfo) {
      ...
      const inputUser = userInfo || { username, password };

      const ajaxData = async (inputUser: UserType) => {
        try {
          // const user: UserType = await authService.login(inputUser);
          const resData: any = await authService.loginAccount(inputUser);

          if (isMounted && resData.code === 0) {
            const user = resData.state;
            dispatch(login(user));
          ...
          if (resData.code === -1) {
            console.log(';;user-login-err, ', resData.state);

            dispatch({ type: LOGOUT });
            dispatch({ type: AUTHENTICATE_ERROR });
            setError(resData.state);
            setIsLoading(false);
          }
        } catch (err) {
  ...
  useEffect(() => {
    // ! 仅用于测试，以admin管理员的方式自动登录
    // handleSubmitLoginForm(undefined, { username: 'admin', password: '111111' });
    // setUsername('admin');
    // setPassword('111111');
              ...
              <div className='p-4'>
                ...
                <Form className='mt-3' id='loginForm'>
                  ...
                  <div className='text-center'>
                    没有帐号？ <Link to='/id/register'>去注册</Link>
                  </div>
                  <LoginHelperMessage
