...
/** 默认false，typeof window === 'undefined' */
...
type Props = {
  rtl: boolean;
  /** 对应editor组件的this.state.blockMenuOpen */
  isActive: boolean;
  commands: Record<string, any>;
  dictionary: typeof baseDictionary;
  view: EditorView;
  /** 对应editor组件的this.state.blockMenuSearch */
  search: string;
...
class BlockMenu extends React.Component<Props, State> {
  ...
  handleKeyDown = (event: KeyboardEvent) => {
    // console.log(';;handleKeyDown start-b-menu-isActive',this.props.isActive)
  ...
  insertBlock(item) {
    ...
    console.log(';insertBlock-item, ', item);
  ...
  get filtered() {
    ...
    const filtered = items.filter((item) => {
      ...
      if (
        item.name &&
        !commands[item.name] &&
        // fix: Table option not appearing in block menu
    ...
    // this block literally just trims unnecessary separators from the results
  ...
  render() {
    // console.log(';;pps4 BlockMenu, ', this.props, this.state);
