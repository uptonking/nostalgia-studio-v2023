...
export default <P>(providers: (P | Promise<P>)[]) => {
  ...
  const getFulfilledProviders = async () => {
    const promises = providers.map((result) => Promise.resolve(result));
    const results = await waitForAllPromises<P>(promises);
  ...
  /** 高阶函数，返回一个方法，会从provider中取出指定方法，然后通过method.apply(args)执行方法 */
  ...
  /**
   * Run a method in the provider which expects to return a list of items
   * @param methodName
   * @param args
   */
