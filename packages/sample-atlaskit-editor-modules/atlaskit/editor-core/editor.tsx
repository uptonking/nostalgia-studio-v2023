...
import type { ExtensionProvider, Transformer } from '../editor-common';
import {
  BaseTheme,
  ProviderFactory,
  WidthProvider,
...
import { akEditorFullPageDefaultFontSize } from '../editor-shared-styles';
import EditorActions from './actions';
import { ReactEditorView, getUiComponent } from './create-editor';
...
import type { EditorProps, ExtensionProvidersProp } from './types/editor-props';
...
// import { name, nextMajorVersion, version } from './version-wrapper';
...
/** 基于react实现的编辑器组件，可通过props指定使用带布局的Editor如full-page或comment */
export default class Editor extends React.Component<EditorProps, State> {
  ...
  /** provider管理器，也是一个事件管理器 */
  private providerFactory: ProviderFactory;
  /** 作为EditorContext的value，提供了编辑器更新方法 */
  private editorActions: EditorActions;
  // private createAnalyticsEvent?: CreateUIAnalyticsEvent;
  private createAnalyticsEvent?: any;
  ...
  componentDidUpdate(prevProps: EditorProps) {
    ...
    const IsQuickInsertOrExtentensionProviderChanged =
      (extensionProviders &&
    ...
    if (IsQuickInsertOrExtentensionProviderChanged) {
  ...
  // #region /folded deprecationWarnings
  private deprecationWarnings(props: EditorProps) {
    ...
    if (
      ...
      console.warn(
        `Advanced table options are deprecated (except isHeaderRowRequired) to continue using advanced table features
        use - <Editor allowTables={{ advanced: true }} /> [Will be changed in editor-core@ {nextVersion}]`,
      );
    }
  }
  // #endregion /folded deprecationWarnings

  /** 返回包含多个方法的映射表，可以方便地操作所有extensionProviders */
  ...
  /** 将quickInsertProvider和extensionQuickInsertProvider合并 */
  ...
  /** 用传入参数更新this.providerFactory对象保存的providers映射表，也会执行注册过的cb */
  ...
  render() {
    ...
    const EditorVariantComp = getUiComponent(this.props.appearance!);
                        ...
                        >
                          <EditorVariantComp
                            ...
                            enableToolbarMinWidth={
                              ...
                              this.props.featureFlags?.toolbarMinWidthOverflow
