...
import type { PluginConfig as TablesPluginConfig } from '../plugins/table/types';
...
export interface EditorProps {
  ...
  */
  ...
  /** Optionally adds an element (eg: an icon) at the start of the editor's primary toolbar.
   * If not specified, the primary toolbar spans the entire width. */
  ...
  /** Configure allowed blocks in the editor,
   * currently only supports `heading`, `blockquote`, `hardBreak` and `codeBlock`. */
  allowBlockType?: BlockTypePluginOptions['allowBlockType'];

  /** Whether or not you want to allow Action and Decision elements in the editor.
   * You can currently only enable both or disable both.
   ...
   * Use object form to enable breakout for layouts,
   * and to enable the newer layouts - left sidebar & right sidebar  */
  ...
  /** Content to appear in the context panel.
   * Displays as a right sidebar in the full-page appearance.
   * You'll want to pass it a `ContextPanel` component, and your content as its children. */
  ...
  /** Set to configure media features.
   * Media single refers to the embedded version of media, which is probably what you want.
   * Media group refers to a filmstrip, thumbnail view of media files which was used in Stride. */
  media?: MediaOptions;

  /** Set to disable text formatting styles. If not specified, they will be all enabled by default.
   * Code here refers to inline code.
  ...
  /** Default placeholder text to be displayed if line is empty but the document content is not.
   * e.g. 'Type / to insert content' */
  placeholderHints?: string[];

  /** Default placeholder text to be displayed when a bracket '{' is typed and the line is empty.
   * e.g. 'Did you mean to use '/' to insert content?' */
  ...
  /** DOM element used to help calculate correct positioning of popup menus
   * to make sure they dont go offscreen. */
  popupsBoundariesElement?: HTMLElement;
  /** Specify when mount point is different from scrollable element
   * so popup menus can be positioned according the scrollable element. */
...
/** Enables re-providing of AnalyticsContext for all ReactNodeViews. */
  ...
  /**
   * 用来直接添加自定义ak-EditorPlugin，会添加在createPluginList()创建内置插件之后。
  ...
  dangerouslyAppendPlugins?: {
    /** Do not use outside of Editor team. */
