...
import { defaultSchema } from '../adf-schema';
import { Transformer } from '../editor-common';
...
const mdToPmMapping = {
  ...
  heading: {
    block: 'heading',
    attrs: (tok: any) => ({ level: Number(tok.tag.slice(1)) }),
  },
  ...
  ordered_list: {
    block: 'orderedList',
    attrs: (tok: any) => ({ order: Number(tok.attrGet('order')) || 1 }),
  },
