...
const prosemirrorPluginLoader = (id: string) => {
  ...
  > {
    ...
    /** 每次注册新的prosemirrorPlugins，prosemirrorPlugins的值都被更新 */
    override main() {
      this.updateContext({
        prosemirrorPlugins: this.context.prosemirrorPlugins.concat(
          ...this.options.plugins(this.context),
        ),
      });
    }
  };
...
export const createProsemirrorPlugin = (
  id: string,
  plugins: (ctx: LoadPluginContext) => Plugin[],
): Atom => {
