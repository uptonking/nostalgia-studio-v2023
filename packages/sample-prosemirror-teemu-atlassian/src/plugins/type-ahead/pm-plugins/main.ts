...
export function createInitialPluginState(
  ...
  console.log(';;plugin-typeAhead-state-creating');
...
/** 创建typeAhead对应的pm-plugin，更新state的过程类似redux的reducer */
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, pluginState, _oldState, state) {
        console.log(';;plugin-typeAhead-state.apply()');
    ...
    view() {
      console.log(';;plugin-typeAhead-view()');

      return {
        /** Called whenever the editorView's state is updated. */
        update(editorView) {
          console.log(';;plugin-typeAhead-view.update()');
