// 、本文件只导出了Preset一个class，其他export都是类型定义

/** 提供存放和操作所有plugins配置对象的数据结构 */
export class Preset<T extends { name: string }> {
  /** 一个数组，用来存放所有plugins的所有配置对象 */
  ...
  /** 返回所有满足条件的plugins数组，其中去除参数中指定的plugins */
