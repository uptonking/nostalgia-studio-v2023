...
export class APIProvider {
  ...
  emit<T>(path: string, action: any) {
    if (!this.socket)
      throw Error('APIProvider: trying to emit with empty socket');
  ...
  on<T>(actionType: any, fn: (data: T) => void) {
    if (!this.socket)
      throw Error('APIProvider: trying to subscribe to empty socket');
    this.socket?.on(actionType, fn);
  }

  off<T = undefined>(
    // actionType: EditorSocketActionType,
    actionType: any,
    fn: (data: T) => void,
  ) {
    if (!this.socket)
      throw Error('APIProvider: trying to unsubscribe from empty socket');
