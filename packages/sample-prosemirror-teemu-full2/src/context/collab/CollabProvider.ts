...
import { ECollabAction } from '../../shared/collab-socket';
...
// } from 'shared/collab-socket';
// import { ISaveCollabStepsParams } from 'shared/shared';
...
export class CollabProvider {
  ...
  async joinCollabSession() {
    ...
    try {
      const { version } = await this.apiProvider.post<any>(this.joinURL, {
        userId: this.userId,
      });
  ...
  async leaveCollabSession() {
    try {
      ...
      this.apiProvider.off(
        ECollabAction.COLLAB_USERS_CHANGED,
        this.onUsersChanged,
      );
      this.apiProvider.off(
        ECollabAction.COLLAB_CLIENT_EDIT,
        this.onReceiveEdit,
      );
      this.apiProvider.off(
        ECollabAction.COLLAB_SERVER_UPDATE,
  ...
  onReceiveEdit = (data: any) => {
    ...
    if (version !== expectedVersion) {
      throw Error(
        `Version ${version} received when expected: ${expectedVersion}`,
      );
    }
