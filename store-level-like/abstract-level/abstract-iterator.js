...
import { getCallback, getOptions } from './lib/common';
...
class CommonIterator {
  constructor(db, options, legacy) {
    ...
    this[kAbortOnClose] = Boolean(options.abortOnClose);
  ...
  next(callback) {
    ...
    if (this[kClosing]) {
      ...
      this.nextTick(
        callback,
        new ModuleError(
          'Iterator is busy: cannot call next() until previous call has completed',
          {
  ...
  nextv(size, options, callback) {
    ...
    if (this[kClosing]) {
      this.nextTick(
        callback,
        new ModuleError(
          'Iterator is not open: cannot call nextv() after close()',
          {
            code: 'LEVEL_ITERATOR_NOT_OPEN',
          },
        ),
      ...
      this.nextTick(
        callback,
        new ModuleError(
          'Iterator is busy: cannot call nextv() until previous call has completed',
          {
            code: 'LEVEL_ITERATOR_BUSY',
          },
        ),
  ...
  all(options, callback) {
    ...
    if (this[kClosing]) {
      this.nextTick(
        callback,
        new ModuleError(
          'Iterator is not open: cannot call all() after close()',
          {
            code: 'LEVEL_ITERATOR_NOT_OPEN',
          },
        ),
      ...
      this.nextTick(
        callback,
        new ModuleError(
          'Iterator is busy: cannot call all() until previous call has completed',
          {
            code: 'LEVEL_ITERATOR_BUSY',
          },
        ),
  ...
  _all(options, callback) {
    ...
    const nextv = () => {
      ...
      const size =
        this[kLimit] < Infinity ? Math.min(1e3, this[kLimit] - count) : 1e3;
  ...
  seek(target, options) {
    ...
    if (this[kClosing]) {
      ...
      throw new ModuleError(
        'Iterator is busy: cannot call seek() until next() has completed',
        {
          code: 'LEVEL_ITERATOR_BUSY',
        },
      );
  ...
  close(callback) {
    ...
    if (this[kClosed]) {
      ...
      if (!this[kWorking]) {
        ...
        cb(
          new ModuleError('Aborted on iterator close()', {
            code: 'LEVEL_ITERATOR_NOT_OPEN',
          }),
        );
      }
...
export class AbstractIterator extends CommonIterator {
  ...
  [kHandleOne](err, key, value) {
    ...
    try {
      key =
        this[kKeys] && key !== undefined
          ? this[kKeyEncoding].decode(key)
          : undefined;
      value =
        this[kValues] && value !== undefined
          ? this[kValueEncoding].decode(value)
          : undefined;
    } catch (err) {
...
export class AbstractKeyIterator extends CommonIterator {
  constructor(db, options) {
  ...
  [kHandleMany](err, keys) {
    ...
    try {
      for (let i = 0; i < keys.length; i++) {
        const key = keys[i];
        keys[i] =
          key !== undefined ? this[kKeyEncoding].decode(key) : undefined;
      }
    } catch (err) {
...
export class AbstractValueIterator extends CommonIterator {
  constructor(db, options) {
    super(db, options, false);
  }
  ...
  [kHandleOne](err, value) {
    ...
    try {
      value =
        value !== undefined ? this[kValueEncoding].decode(value) : undefined;
    } catch (err) {
