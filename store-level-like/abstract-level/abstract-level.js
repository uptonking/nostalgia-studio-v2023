...
import { Transcoder } from 'level-transcoder';
import ModuleError from 'module-error';

import { AbstractIterator } from './abstract-iterator';
import abstractSublevelFactory from './lib/abstract-sublevel';
import { getCallback, getOptions } from './lib/common';
import { DefaultChainedBatch } from './lib/default-chained-batch';
import {
  DefaultKeyIterator,
  DefaultValueIterator,
} from './lib/default-kv-iterator';
import {
  DeferredIterator,
  DeferredKeyIterator,
...
export class AbstractLevel extends EventEmitter {
  ...
  keyEncoding(encoding) {
    return this[kTranscoder].encoding(
      encoding != null ? encoding : this[kKeyEncoding],
    );
  }

  valueEncoding(encoding) {
    return this[kTranscoder].encoding(
      encoding != null ? encoding : this[kValueEncoding],
    );
  }
  ...
  get(key, options, callback) {
    ...
    this._get(
      this.prefixKey(keyEncoding.encode(key), keyFormat),
      options,
      (err, value) => {
        if (err) {
        ...
        try {
          ...
          return callback(
            new ModuleError('Could not decode value', {
  ...
  getMany(keys, options, callback) {
    ...
    if (!Array.isArray(keys)) {
      this.nextTick(
        callback,
        new TypeError("The first argument 'keys' must be an array"),
      );
      return callback[kPromise];
    }
    ...
    this._getMany(mappedKeys, options, (err, values) => {
      ...
      try {
        ...
        return callback(
          new ModuleError(
            `Could not decode one or more of ${values.length} value(s)`,
            {
              code: 'LEVEL_DECODE_ERROR',
              cause: err,
            },
  ...
  batch(operations, options, callback) {
    ...
    if (!Array.isArray(operations)) {
      this.nextTick(
        callback,
        new TypeError("The first argument 'operations' must be an array"),
      );
      return callback[kPromise];
    }
    ...
    for (let i = 0; i < operations.length; i++) {
      if (typeof operations[i] !== 'object' || operations[i] === null) {
        this.nextTick(
          callback,
          new TypeError('A batch operation must be an object'),
        );
        return callback[kPromise];
      }
...
export const AbstractSublevel = abstractSublevelFactory({ AbstractLevel });
...
const formats = function (db) {
  return Object.keys(db.supports.encodings).filter((k) =>
    Boolean(db.supports.encodings[k]),
  );
};
