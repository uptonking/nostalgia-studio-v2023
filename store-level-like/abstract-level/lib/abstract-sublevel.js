...
} from './abstract-sublevel-iterator';
...
export default function ({ AbstractLevel }) {
  class AbstractSublevel extends AbstractLevel {
    ...
    constructor(db, name, options) {
      ...
      const { separator, manifest, ...forward } =
    ...
    _iterator(options) {
      ...
      const unfix = this[kUnfix].get(
        this[kPrefix].utf8.length,
        options.keyEncoding,
      );
...
class MultiFormat {
  constructor(key) {
    ...
    this.buffer = Buffer
      ? Buffer.from(this.view.buffer, 0, this.view.byteLength)
      : {};
  }
}
