...
exports.args = function (test, testCommon) {
      ...
      db.batch(
        ...
        assertAsync(function (err) {
          ...
          t.is(
            ...
            'correct error message (callback)',
          );
        }),
      );

      db.batch(
        [{ key: 'key', value: 'value', type: 'foo' }],
        assertAsync(function (err) {
          t.is(err && err.name, 'TypeError');
          t.is(
            err && err.message,
  ...
  [null, undefined, 1, true].forEach(function (operation) {
    ...
    test(
      'test batch() with ' + type + ' operation',
      assertAsync.ctx(function (t) {
        t.plan(5);
        ...
        db.batch(
          ...
          assertAsync(function (err) {
            ...
            t.is(
              err && err.message,
              'A batch operation must be an object',
              'correct error message (callback)',
            );
          }),
        );
...
exports.batch = function (test, testCommon) {
  ...
  for (const encoding of ['utf8', 'buffer', 'view']) {
    ...
    test(`empty values in batch with ${encoding} valueEncoding`, async function (t) {
      ...
      await db.open();
      await db.batch(
        values.map((value, i) => ({ type: 'put', key: String(i), value })),
      );
