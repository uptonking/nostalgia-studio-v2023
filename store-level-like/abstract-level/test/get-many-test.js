...
exports.args = function (test, testCommon) {
  test(
    ...
    assertAsync.ctx(function (t) {
      // Add 1 assertion for every assertAsync()
      t.plan(6);

      db.getMany(
        'foo',
        assertAsync(function (err) {
          t.is(err.name, 'TypeError');
          t.is(
            err && err.message,
            "The first argument 'keys' must be an array",
          );
        }),
      );
      db.getMany(
      ...
      for (const { name, key } of illegalKeys) {
        db.getMany(
          ...
          assertAsync(function (err) {
            ...
            t.is(
              err && err.code,
              'LEVEL_INVALID_KEY',
              name + ' - correct error code (callback)',
            );
          }),
        );
        ...
        db.getMany(
          ...
          assertAsync(function (err) {
            t.ok(
              ...
              name + ' - is Error (callback, second key)',
            );
            t.is(
              err && err.code,
              'LEVEL_INVALID_KEY',
              name + ' - correct error code (callback, second key)',
            );
          }),
        );
        ...
        db.getMany([key]).catch(function (err) {
          ...
          t.is(
            err.code,
            'LEVEL_INVALID_KEY',
            name + ' - correct error code (promise)',
          );
        });
        ...
        db.getMany(['valid', key]).catch(function (err) {
          t.ok(
            err instanceof Error,
            name + ' - is Error (promise, second key)',
          );
          t.is(
            err.code,
            'LEVEL_INVALID_KEY',
            name + ' - correct error code (promise, second key)',
          );
        });
      }
    }),
...
exports.getMany = function (test, testCommon) {
        ...
        db.getMany(['foo'], {}, function (err, values) {
          ...
          db.getMany(
            ...
            function (err, values) {
              t.error(err);
              t.is(
                values && typeof values[0],
                'string',
                'should be string if not buffer',
              );
              t.same(values, ['bar']);
              t.end();
            },
  ...
  test(
    'test empty getMany()',
    assertAsync.ctx(function (t) {
      const encodings = Object.keys(db.supports.encodings).filter(
        (k) => db.supports.encodings[k],
      );
      ...
      for (const valueEncoding of encodings) {
        db.getMany(
          [],
          { valueEncoding },
          assertAsync(function (err, values) {
            t.ifError(err);
            t.same(values, []);
          }),
        );
  ...
  test('test getMany() with promise', async function (t) {
    ...
    t.same(await db.getMany(['foo', 'beep'], { valueEncoding: 'utf8' }), [
      'bar',
      'boop',
    ]);
    t.same(await db.getMany(['beep', 'foo'], { valueEncoding: 'utf8' }), [
      'boop',
      'bar',
    ]);
    t.same(
      await db.getMany(['beep', 'foo', 'nope'], { valueEncoding: 'utf8' }),
      ['boop', 'bar', undefined],
    );
  ...
  test('test getMany() on closed db', function (t) {
    ...
    for (const keys of [['foo'], []]) {
      ...
      db.open(function (err) {
        ...
        db.close(
          assertAsync.with(t, function (err) {
            t.ifError(err);
