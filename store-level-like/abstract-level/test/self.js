...
test('test open() extensibility when open', function (t) {
  ...
  const spy = sinon.spy(function (options, cb) {
    this.nextTick(cb);
  });
  const Test = implement(AbstractLevel, { _open: spy });
  const test = new Test({ encodings: { utf8: true } });
...
test('test opening explicitly gives a chance to capture an error', function (t) {
  ...
  const spy = sinon.spy(function (options, cb) {
    this.nextTick(cb, new Error('_open error'));
  });
  const Test = implement(AbstractLevel, { _open: spy });
  const test = new Test({ encodings: { utf8: true } });
...
test('test close() extensibility when open', function (t) {
  ...
  const spy = sinon.spy(function (cb) {
    this.nextTick(cb);
  });
...
test('test chained batch() extensibility', function (t) {
  ...
  test.once('open', function () {
    ...
    test
      ...
      .write(expectedOptions, expectedCb);
...
test('test chained batch() (custom _chainedBatch) extensibility', function (t) {
  ...
  test.once('open', function () {
    ...
    t.equal(
      spy.getCall(0).thisValue,
      test,
      '`this` on _chainedBatch() was correct',
    );
...
test('test AbstractChainedBatch expects a db', function (t) {
  ...
  try {
    ...
    t.is(
      err.message,
      'The first argument must be an abstract-level database, received undefined',
    );
  }
});
...
test('test clear() extensibility', function (t) {
  ...
  db.once('open', function () {
    ...
    call(
      [{ reverse: true, limit: 0 }, callback],
      { keyEncoding: 'utf8', reverse: true, limit: 0 },
      true,
    );
    call([{ reverse: 1 }, callback], {
      keyEncoding: 'utf8',
      reverse: true,
      limit: -1,
    });
    call([{ reverse: null }, callback], {
      keyEncoding: 'utf8',
      reverse: false,
      limit: -1,
    });
    ...
    function call(args, expectedOptions, shouldSkipCall) {
      ...
      if (!shouldSkipCall) {
        ...
        t.is(
          typeof spy.getCall(0).args[1],
          'function',
          'got callback argument',
        );
      }
