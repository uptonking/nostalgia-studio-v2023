...
test('sublevel prefix and options', function (t) {
  ...
  t.test('legacy sublevel(down) options', function (t) {
    t.throws(
      () => new NoopLevel().sublevel('foo', 'bar'),
      (err) => err.code === 'LEVEL_LEGACY',
    );
    t.throws(
      () => new NoopLevel().sublevel('foo', { open: () => {} }),
      (err) => err.code === 'LEVEL_LEGACY',
    );
    t.end();
  });
...
test('sublevel.prefixKey()', function (t) {
  ...
  t.same(
    sub.prefixKey(textEncoder.encode(''), 'view'),
    textEncoder.encode('!test!'),
  );
  t.same(
    sub.prefixKey(textEncoder.encode('a'), 'view'),
    textEncoder.encode('!test!a'),
  );
...
test('sublevel manifest and parent db', function (t) {
  ...
  t.test('sublevel does not support additionalMethods', function (t) {
    ...
    t.is(
      typeof sub.foo,
      'undefined',
      'AbstractSublevel does not expose method',
    );
    t.end();
  });
      ...
      function (t) {
        ...
        class MockIterator extends Ctor {
          _next(callback) {
            this.nextTick(
              callback,
              new Error('next() error from parent database'),
            );
          }
        }
  ...
  for (const [mode, def] of [
    ...
    t.test(
      ...
      function (t) {
        ...
        const db = new MockLevel({
          encodings: { buffer: true, view: true, utf8: true },
        });
        const sub = db.sublevel('test', { keyEncoding: 'buffer' });
    ...
    t.test(
      ...
      function (t) {
        ...
        const prefixedKey = new Uint8Array(
          prefix.byteLength + testKey.byteLength,
        );
