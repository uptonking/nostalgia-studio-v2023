...
import createKeyRange from './util/key-range';
    ...
    if (!this[kOptions].reverse) {
      ...
      const complete = () => {
        ...
        for (let i = 0; i < length; i++) {
          ...
          entries[i] = [
            this[kOptions].keys && key !== undefined
              ? deserialize(key)
              : undefined,
            this[kOptions].values && value !== undefined
              ? deserialize(value)
              : undefined,
          ];
        }
      ...
      if (this[kOptions].values) {
        store.getAll(keyRange, size < Infinity ? size : undefined).onsuccess = (
          ev,
        ) => {
      ...
      const method =
        !this[kOptions].values && store.openKeyCursor
          ? 'openKeyCursor'
          : 'openCursor';
