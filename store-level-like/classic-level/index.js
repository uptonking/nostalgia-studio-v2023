...
export class ClassicLevel extends AbstractLevel {
  constructor(location, options, _) {
    ...
    if (typeof location !== 'string' || location === '') {
      throw new TypeError(
        "The first argument 'location' must be a non-empty string",
      );
    }
  ...
  approximateSize(start, end, options, callback) {
    ...
    if (this.status === 'opening') {
      ...
      this.nextTick(
        callback,
        new ModuleError('Database is not open: cannot call approximateSize()', {
  ...
  compactRange(start, end, options, callback) {
    ...
    if (this.status === 'opening') {
      ...
      this.nextTick(
        callback,
        new ModuleError('Database is not open: cannot call compactRange()', {
          code: 'LEVEL_DATABASE_NOT_OPEN',
        }),
      );
    } else {
