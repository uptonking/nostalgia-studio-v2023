...
const verify = function (location, compression, t) {
  du(location, function (err, size) {
    ...
    if (compression) {
      t.ok(
        size < dataSize,
        'on-disk size (' + size + ') is less than data size (' + dataSize + ')',
      );
    } else {
      t.ok(
        size >= dataSize,
    ...
    db.open(function (err) {
      ...
      each(
        ...
        cycle.bind(
          ...
          true,
          t,
          delayed.delayed(verify.bind(null, db.location, true, t), 0.01),
        ),
      );
    });
  });
      ...
      db.open({ compression: false }, function (err) {
        ...
        each(
          ...
          cycle.bind(
            ...
            false,
            t,
            delayed.delayed(verify.bind(null, db.location, false, t), 0.01),
          ),
        );
      });
    },
