...
export class EntryStream<
  K,
  V,
  TDatabase = AbstractLevel<any, any, any>,
> extends LevelReadStream<{ key: K; value: V }, TDatabase> {
  /**
  ...
  constructor(
    ...
    options?:
      | (ReadStreamOptions &
          Omit<AbstractIteratorOptions<K, V>, 'keys' | 'values'>)
      | undefined,
  );
...
export class KeyStream<
  K,
  TDatabase = AbstractLevel<any, any, any>,
> extends LevelReadStream<K, TDatabase> {
  /**
  ...
  constructor(
    db: TDatabase,
    options?: (ReadStreamOptions & AbstractKeyIteratorOptions<K>) | undefined,
  );
}
...
export class ValueStream<
  ...
  V,
  TDatabase = AbstractLevel<any, any, any>,
> extends LevelReadStream<V, TDatabase> {
  /**
  ...
  constructor(
    db: TDatabase,
    options?:
      | (ReadStreamOptions & AbstractValueIteratorOptions<K, V>)
      | undefined,
  );
}
