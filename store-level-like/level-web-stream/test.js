...
test('EntryStream', async function (t) {
  t.same(
    await concat(new EntryStream(db)),
...
test('ValueStream', async function (t) {
  t.same(
    await concat(new ValueStream(db)),
    data.map((kv) => kv.value),
  );
});
...
test('BatchStream: basic put operation', async function (t) {
  ...
  await from([{ type: 'put', key: 'a', value: '1' }]).pipeTo(
    new BatchStream(db),
  );
  await from([{ key: 'b', value: '2' }]).pipeTo(
    new BatchStream(db, { type: 'put' }),
  );
...
test('BatchStream: operation type takes precedence', async function (t) {
  ...
  await from([{ type: 'put', key: 'a', value: '2' }]).pipeTo(
    new BatchStream(db, { type: 'del' }),
  );
...
test('BatchStream: uses batches of fixed size', async function (t) {
  ...
  t.same(
    batchCalls.map((args) => args[0].length),
    [5, 5, 5, 2],
    'batch size is fixed',
  );
});
