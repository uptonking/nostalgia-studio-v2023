...
class ManyLevelGuest extends AbstractLevel {
  ...
  createRpcStream(opts) {
    ...
    function oncallback(res) {
      ...
      if (res.error)
        req.callback(
          new ModuleError('Could not get value', { code: res.error }),
        );
    ...
    function ongetmanycallback(res) {
      ...
      if (res.error)
        req.callback(
          new ModuleError('Could not get values', { code: res.error }),
        );
      else
        req.callback(
          null,
          res.values.map((v) => normalizeValue(v.value)),
        );
    }
  }
    ...
    for (const enc of ['keyEncoding', 'valueEncoding']) {
      if (db[enc]('buffer').name !== 'buffer') {
        throw new ModuleError(
          `Database must support non-transcoded 'buffer' ${enc}`,
          {
  ...
  _open(options, cb) {
    if (this[kRemote]) {
      ...
      pipeline(remote, this.connect(), remote, () => {});
...
class Iterator extends AbstractIterator {
  constructor(db, options) {
    ...
    super(db, { ...options, abortOnClose: true });
    ...
    if (this[kRequest].consumed >= this.limit || this[kErrored]) {
      ...
      if (next.error) {
        ...
        return this.nextTick(
          callback,
          new ModuleError('Could not read entry', {
            code: next.error,
          }),
        );
      }
