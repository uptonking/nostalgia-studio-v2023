...
import lpstream from '@vweevers/length-prefixed-stream';

import { input, output } from './tags';

const rangeOptions = new Set(['gt', 'gte', 'lt', 'lte']);
const encodingOptions = Object.freeze({
  keyEncoding: 'buffer',
...
class ManyLevelHost {
  constructor(db, options) {
    ...
    this[kOptions].readonly = Boolean(this[kOptions].readonly);
    this[kOptions].preput =
      this[kOptions].preput ||
      function (key, val, cb) {
        cb(null);
      };
    this[kOptions].predel =
      this[kOptions].predel ||
      function (key, cb) {
        cb(null);
      };
    this[kOptions].prebatch =
      this[kOptions].prebatch ||
      function (ops, cb) {
        cb(null);
      };
    this[kOptions].events = (this[kOptions].events || getEvents(db)).filter(
      safeEvent,
    );
  }
...
function createRpcStream(db, options, streamOptions) {
  ...
  function ready(err) {
    ...
    decode.on('data', function (data) {
      ...
      switch (tag) {
        case input.get:
          return onget(req);
        case input.put:
          return readonly ? onreadonly(req) : onput(req);
        case input.del:
          return readonly ? onreadonly(req) : ondel(req);
        case input.batch:
          return readonly ? onreadonly(req) : onbatch(req);
        case input.iterator:
          return oniterator(req);
        case input.iteratorClose:
          return oniteratorclose(req);
        case input.iteratorAck:
          return oniteratorack(req);
        case input.iteratorSeek:
          return oniteratorseek(req);
        case input.clear:
          return readonly ? onreadonly(req) : onclear(req);
        case input.getMany:
          return ongetmany(req);
      }
    });
    ...
    function onreadonly(req) {
      callback(
        req.id,
        new ModuleError('Database is readonly', { code: 'LEVEL_READONLY' }),
      );
    }
...
class Iterator {
  ...
  next(first) {
    ...
    if (first) {
      ...
      size = Math.min(
        size,
        Math.max(16, Math.round(room / this[kMaxItemLength])),
      );
