...
export function encodeRequest(clientRows: any) {
  // Index by shard
  clientRows = _.groupBy(clientRows, (row: any) =>
    row._id.substr(0, shardLength),
  );
...
export function encodeResponse(serverRows: any, encodedRequest: any) {
  // Index by shard
  serverRows = _.groupBy(serverRows, (row: any) =>
    row._id.substr(0, shardLength),
  );

  // Include any that are in encoded request but not present
  for (const key in encodedRequest) {
  ...
  const response = _.pickBy(
    serverRows,
    (rows: any, key: any) => hashRows(rows) !== encodedRequest[key],
  );
...
function hashRows(rows: any) {
  ...
  for (const row of _.sortBy(rows, '_id')) {
