...
// Select appropriate local database, prefering IndexedDb, then LocalStorageDb, then MemoryDb
export function autoselectLocalDb(options: any, success: any, error: any) {
  ...
  if (browser.ios || browser.safari) {
    ...
    return new IndexedDb(options, success, (err: any) => {
      console.log(
        'Failed to create IndexedDb: ' + (err ? err.message : undefined),
      );
  ...
  if (
    ...
    if (typeof window['openDatabase'] === 'function') {
      ...
      return new WebSQLDb(options, success, (err: any) => {
        console.log(
          'Failed to create WebSQLDb: ' + (err ? err.message : undefined),
        );
...
function processNearOperator(selector: any, list: any) {
  for (var key in selector) {
    ...
    if (value != null && value['$near']) {
      ...
      list = _.filter(
        list,
        (doc: any) => doc[key] && doc[key].type === 'Point',
      );
      ...
      if (value['$near']['$maxDistance']) {
        distances = _.filter(
          distances,
          (item: any) => item.distance <= value['$near']['$maxDistance'],
        );
      }
    ...
    if (value != null && value['$geoIntersects']) {
      ...
      list = _.filter(list, function (doc: any) {
        ...
        if (doc[key].type === 'Point') {
          ...
          for (const line of doc[key].coordinates) {
