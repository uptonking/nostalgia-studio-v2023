...
describe('RemoteDb', function () {
  ...
  it('calls GET for find', function (done: any) {
    ...
    return this.col
      .find({ a: 1 }, { limit: 10, sort: ['b'] })
      .fetch(success, () => assert.fail());
  });
  ...
  it('calls POST for find that is too big', function (done: any) {
    ...
    return this.col
      .find({ a: longStr }, { limit: 10, sort: ['b'] })
      .fetch(success, () => assert.fail());
  });
  ...
  it('calls GET for findOne', function (done: any) {
    ...
    return this.col.findOne({ a: 1 }, { sort: ['b'] }, success, () =>
      assert.fail(),
    );
  });
  ...
  it('calls PATCH for upsert with base', function (done: any) {
    const success = (data: any) => {
      ...
      assert.deepEqual(this.httpCall.data, {
        doc: { _id: '0', _rev: 1, x: 2 },
        base: { _id: '0', _rev: 1, x: 1 },
      });
  ...
  it('calls POST quickfind for find if localData passed', function (done: any) {
    const success = (data: any) => {
      ...
      assert.equal(
        this.httpCall.url,
        'http://someserver.com/scratch/quickfind',
      );
      assert.deepEqual(
        this.httpCall.params,
        { client: 'clientid' },
    ...
    return this.col
      .find({ a: 1 }, { limit: 10, sort: ['b'], localData })
      .fetch(success, () => assert.fail());
  });
