...
describe('quickfind', function () {
  before(function (this: any) {
    this.completeTest = function (clientRows: any, serverRows: any, sort: any) {
      ...
      const encodedResponse = quickfind.encodeResponse(
        serverRows,
        encodedRequest,
      );
      const decodedResponse = quickfind.decodeResponse(
        encodedResponse,
        clientRows,
        sort,
      );
  ...
  it('replaces', function (this: any) {
    return this.completeTest(
      ...
      [this.row1, this.row2a, this.row3],
    );
  });

  return it('sorts', function (this: any) {
    return this.completeTest(
      [this.row2, this.row1, this.row3],
      [this.row2, this.row1, this.row3],
      ['a'],
    );
  });
