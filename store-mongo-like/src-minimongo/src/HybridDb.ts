...
export class HybridCollection<T extends Doc>
  implements MinimongoBaseCollection<T>
{
  name: string;
    ...
    this.localCol.pendingUpserts!((upserts: any) => {
      this.localCol.pendingRemoves!((removes: any) => {
        const step2 = (localData: any) => {
          ...
          const remoteSuccess = (remoteData: any) => {
            ...
            if (options.cacheFind) {
              ...
              const cacheSuccess = () => {
                ...
                return this.localCol
                  .find(selector, options)
                  .fetch(localSuccess2, error);
              };
              ...
              if (removes.length > 0) {
                const removesMap = _.fromPairs(
                  _.map(removes, (id: any) => [id, id]),
                );
                data = _.filter(
                  remoteData,
                  (doc: any) => !_.has(removesMap, doc._id),
                );
              }
              ...
              if (upserts.length > 0) {
                ...
                data = _.filter(
                  data,
                  (doc: any) => !_.has(upsertsMap, doc._id),
                );
          ...
          return this.remoteCol
            .find(selector, remoteOptions)
            .fetch(remoteSuccess, remoteError);
        };
  ...
  upload(success: () => void, error: (err: any) => void) {
      ...
      if (remove) {
        return this.remoteCol.remove(
          ...
          () => {
            return this.localCol.resolveRemove(
              remove,
              () => uploadRemoves(_.tail(removes), success, error),
              error,
            );
          },
          (err: any) => {
    ...
    this.localCol.pendingUpserts((upserts: any) => {
      ...
      if (this.options.sortUpserts) {
        upserts.sort((u1: any, u2: any) =>
          this.options.sortUpserts(u1.doc, u2.doc),
        );
      }
      ...
      return uploadUpserts(
        upserts,
        () => {
          return this.localCol.pendingRemoves(
            (removes: any) => uploadRemoves(removes, success, error),
            error,
          );
        },
        error,
