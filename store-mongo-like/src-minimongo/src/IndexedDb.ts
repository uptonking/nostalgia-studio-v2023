...
export default class IndexedDb implements MinimongoLocalDb {
  ...
  removeCollection(name: any, success: any, error: any) {
    ...
    return this.store.query(
      ...
      {
        index: 'col',
        keyRange: this.store.makeKeyRange({ only: name }),
        onError: error,
      },
    );
  }
...
class IndexedDbCollection<T> implements MinimongoLocalCollection<T> {
  ...
  _findFetch(selector: any, options: any, success: any, error: any) {
    ...
    return this.store.query(
      ...
      {
        index: 'col',
        keyRange: this.store.makeKeyRange({ only: this.name }),
        onError: error,
      },
    );
  }
      ...
      (records: any) => {
        const puts = _.map(items, (item: any, i: any) => {
          ...
          if (item.base !== undefined) {
            ...
            records[i] &&
            records[i].doc &&
            records[i].state === 'cached'
          ) {
            base = records[i].doc;
          } else if (
            records[i] &&
            records[i].doc &&
            records[i].state === 'upserted'
          ) {
            ({ base } = records[i]);
          } else {
            base = null;
    ...
    const step2 = () => {
      ...
      return this.find(selector, options).fetch((results: any) => {
        ...
        return this.store.getBatch(
          ...
          (records: any) => {
            for (
              let i = 0, end = records.length, asc = 0 <= end;
              asc ? i < end : i > end;
              asc ? i++ : i--
            ) {
  ...
  pendingUpserts(success: any, error: any) {
    return this.store.query(
      ...
      {
        index: 'col-state',
        keyRange: this.store.makeKeyRange({ only: [this.name, 'upserted'] }),
        onError: error,
      },
    );
  }
  ...
  pendingRemoves(success: any, error: any) {
    return this.store.query(
      ...
      {
        index: 'col-state',
        keyRange: this.store.makeKeyRange({ only: [this.name, 'removed'] }),
        onError: error,
      },
    );
  }
  ...
  resolveUpserts(upserts: any, success: any, error: any) {
    ...
    return this.store.getBatch(
      ...
      (records: any) => {
        const puts = [];
        for (
          let i = 0, end = upserts.length, asc = 0 <= end;
          asc ? i < end : i > end;
          asc ? i++ : i--
        ) {
          const record = records[i];
  ...
  uncache(selector: any, success: any, error: any) {
    ...
    return this.store.query(
      (matches: any) => {
        ...
        matches = _.filter(
          matches,
          (m: any) => m.state === 'cached' && compiledSelector(m.doc),
        );
  ...
  uncacheList(ids: any, success: any, error: any) {
    ...
    return this.store.query(
      (matches: any) => {
        ...
        matches = _.filter(
          matches,
          (m: any) => m.state === 'cached' && idIndex[m.doc._id],
        );
        const keys = _.map(matches, (m: any) => [this.name, m.doc._id]);
        if (keys.length > 0) {
          return this.store.removeBatch(
