...
export default class LocalStorageDb implements MinimongoDb {
  ...
  removeCollection(name: any, success: any, error: any) {
    if (this.namespace && window.localStorage) {
      const keys = [];
      for (
        let i = 0, end = window.localStorage.length, asc = 0 <= end;
        asc ? i < end : i > end;
        asc ? i++ : i--
      ) {
...
class Collection<T extends Doc> implements MinimongoLocalCollection<T> {
  ...
  loadStorage() {
    ...
    const removeItems = window.localStorage[this.namespace + 'removes']
      ...
      : [];
    return (this.removes = _.fromPairs(
      _.zip(_.map(removeItems, '_id'), removeItems),
    ));
  }
  ...
  _findFetch(selector: any, options: any, success: any, error: any) {
    ...
    if (success != null) {
      return success(
        processFind(
          JSON.parse(JSON.stringify(_.values(this.items))),
          selector,
          options,
        ),
      );
  ...
  _putUpsert(upsert: any) {
    ...
    if (this.namespace) {
      window.localStorage[this.namespace + 'upserts'] = JSON.stringify(
        _.keys(this.upserts),
      );
      window.localStorage[this.namespace + 'upsertbase_' + upsert.doc._id] =
        JSON.stringify(upsert.base);
    }
  }
  ...
  _putRemove(doc: any) {
    ...
    if (this.namespace) {
      window.localStorage[this.namespace + 'removes'] = JSON.stringify(
        _.values(this.removes),
      );
    }
  }
  ...
  uncache(selector: any, success: any, error: any) {
    ...
    for (const item of _.values(this.items)) {
