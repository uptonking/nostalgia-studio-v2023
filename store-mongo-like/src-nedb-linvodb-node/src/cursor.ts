 ...
 */
export function Cursor(this: any, db, query, execFn = undefined) {
  this.db = db;
  this.query = query || {};
  if (execFn) {
    this.execFn = execFn;
...
Cursor.prototype.live = function (query) {
  ...
  // Refresh live query
  const refresh = _.debounce(function (callback = undefined) {
  ...
  function updated(docs) {
    ...
    docs.forEach(function (doc) {
      ...
      const interested =
        self._count || self._ids[doc._id] || document.match(doc, self.query); // _count queries never set _ids
 ...
 */
Cursor.getMatchesStream = function (
  db,
  query,
  sort = undefined,
 ...
 */
Cursor.getIdsForQuery = function (db, query, sort = undefined) {
  const self = db;
  ...
  function match(key, val) {
    ...
    if (key[0] == '$') {
      if (key == '$and' || key == '$or') {
        if (!Array.isArray(val)) {
          throw key + ' operator used without an array';
        }

        const i = val.map(function (q) {
    ...
    if (index && (indexed || !db.options.autoIndexing)) {
      ...
      if (
        typeof val === 'object' &&
        !_.keys(val).some(function (k) {
          return document.comparators[k];
        })
      )
...
Cursor.getSorter = function (sort) {
  const criteria = _.map(sort, function (val, key) {
    return { key: key, direction: val };
  });
  return function (a, b) {
    let criterion;
    let compare;
