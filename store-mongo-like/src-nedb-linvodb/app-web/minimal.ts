...
LinvoDB.defaults.store = { db: BrowserLevel }; // Comment out to use LevelDB instead of level-js
// LinvoDB.dbPath = 'tests/testdata'; // path实参 对应于 idb的objectStore名称的前一部分

// 初始化时传入当前collection的名称
const doc11 = new LinvoDB('doc', { filename: 'tests/testdata/lvDoc.db' });

console.log(';; Doc ', doc11);

// you can use the .insert method to insert one or more documents
doc11.insert({ _id: 'testId', aa: 3 }, (err, newDoc) => {
