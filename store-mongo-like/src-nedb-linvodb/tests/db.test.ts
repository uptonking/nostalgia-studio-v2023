  ...
  */
    ...
    it('Can use indexes for comparison matches', function (done) {
      d.ensureIndex({ fieldName: 'tf' }, function (err) {
        d.insert({ tf: 4 }, function (err, _doc1) {
          d.insert({ tf: [4, 6, 8] }, function (err, _doc2) {
            // matched, test with array
  ...
  describe('Update', function () {
    ...
    it('Can update via modifier function', function (done) {
      d.insert(
        [{ somedata: 'ok' }, { somedata: 'again' }, { somedata: 'againy' }],
        function (err) {
          d.update(
            { somedata: 'ok' },
            function (doc) {
          ...
          function (err, newDoc) {
            ...
            d.insert(
              ...
              function (err) {
                ...
                assert.deepEqual(d.indexes.nonu1.getMatching('yes'), [
                  newDoc._id,
                ]);
                assert.deepEqual(d.indexes.uni.getMatching('willfail'), [
                  newDoc._id,
                ]);
                assert.deepEqual(d.indexes.nonu2.getMatching('yes2'), [
                  newDoc._id,
                ]);
