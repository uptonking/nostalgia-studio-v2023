...
export class Persistence implements PersistenceOptionsProps {
  ...
  constructor(options: PersistenceOptionsProps & { db: Datastore }) {
    ...
    if (
      options.testSerializationHooks === undefined ||
      options.testSerializationHooks
    ) {
  ...
  public async persistCachedDatabaseAsync(): Promise<void> {
    ...
    await storage.crashSafeWriteFileLinesAsync(
      this.filename,
      lines,
      this.modes,
    );
  ...
  setAutocompactionInterval(interval) {
    deprecate(
      (_interval) => this.db.setAutocompactionInterval(_interval),
      'nedb: calling Datastore#persistence#setAutocompactionInterval is deprecated, please use Datastore#setAutocompactionInterval, it will be removed in the next major version.',
    )(interval);
  }
  ...
  stopAutocompaction() {
    deprecate(
      ...
      'nedb: calling Datastore#persistence#stopAutocompaction is deprecated, please use Datastore#stopAutocompaction, it will be removed in the next major version.',
    )();
  }
    ...
    if (dataLength > 0) {
      ...
      if (corruptionRate > this.corruptAlertThreshold) {
        const error: any = new Error(
          `${Math.floor(
            100 * corruptionRate,
          )}% of the data file is corrupt, more than given corruptAlertThreshold (${Math.floor(
  ...
  async loadDatabaseAsync() {
    ...
    // In-memory only datastore
    if (this.inMemoryOnly) return;
    await Persistence.ensureDirectoryExistsAsync(
      path.dirname(this.filename),
      this.modes.dirMode,
    );
    await storage.ensureDatafileIntegrityAsync(
      this.filename,
      this.modes.fileMode,
    );
