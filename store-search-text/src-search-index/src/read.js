...
export default function read(ops, cache) {
  ...
  const DOCUMENT_VECTORS = (...requestedDocs) =>
    Promise.all(
      requestedDocs.map((_id) =>
        ops.fii.STORE.get(['DOC', _id]).catch((e) => null),
      ),
    );
  ...
  const SORT = (results, options) => {
    ...
    const sortFunction = {
      NUMERIC: {
        DESCENDING: (a, b) => Number(b._score) - Number(a._score),
        ASCENDING: (a, b) => Number(a._score) - Number(b._score),
      },
      ALPHABETIC: {
        DESCENDING: (a, b) => {
    ...
    results.map((r) => {
      r._match = r._match.map((m) => {
        weights.forEach((w) => {
          ...
          if (doWeighting) m.SCORE = (w.WEIGHT * Number(m.SCORE)).toFixed(2);
        });
        return m;
      });
  ...
  /** reads queries in a JSON format and then translates them to Promises */
