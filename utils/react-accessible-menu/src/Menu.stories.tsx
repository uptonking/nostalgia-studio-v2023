...
export const UsingHooks = () => {
   ...
   */
  const { Provider, contextProps, focusedItem, focusItem, items, renderProps } =
    useMenu({});
  ...
  const currentlySelected = items.current.find(
    ...
    <Menu
      ...
      renderMenu={({ props, ref }) => (
        <div ref={ref} {...props} className='list'>
          ...
          <button onClick={() => menu.current?.focusItem('Blueberry')}>
            Focus Blueberry
          </button>
        </div>
      )}
    ...
    <Menu
      ...
      renderMenu={({ props, ref }) => (
        <div ref={ref} {...props}>
          ...
          />
          <br />
          Count of additional items:{' '}
          <input
            value={customItemCount}
            onChange={(e) =>
              setCustomItemCount(Math.max(parseInt(e.target.value), 0))
            }
            type='number'
          />
          <br />
