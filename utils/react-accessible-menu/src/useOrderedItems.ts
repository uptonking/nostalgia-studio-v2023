...
export const useOrderedItems = (
  ...
  const focusItem = useCallback(
    (item: RegisteredItem, passive = false) => {
  ...
  const registerItem = useCallback<MenuContextProps['registerItem']>(
    (item) => {
  ...
  const unregisterItem = useCallback<MenuContextProps['unregisterItem']>(
    (id) => {
  ...
  const getFocusedItemIndex = useCallback(() => {
    return items.current.findIndex(
      (item) => item.id === focusedItemRef.current,
    );
  }, []);
  ...
  const moveFocusIndexRelative = useCallback(
    (direction: 'up' | 'down') =>
      findNextFocusIndex(
        getFocusedItemIndex() + (direction === 'up' ? -1 : 1),
        direction,
      ),
  ...
  const moveFocusToStart = useCallback(
    () => findNextFocusIndex(0, 'down'),
    [findNextFocusIndex],
  );

  const moveFocusToEnd = useCallback(
    () => findNextFocusIndex(items.current.length - 1, 'up'),
    [findNextFocusIndex],
  );
